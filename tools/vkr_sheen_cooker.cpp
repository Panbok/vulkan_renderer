...
double full_density(const FullFit &fit, Vec3 direction) {
  const std::array<double, 4> a = full_matrix(fit);
...
struct RectfitObjective {
  ...
  std::array<FullFit, 2> evaluate(const std::array<double, 8> &parameters,
    ...
    for (size_t i = 0; i < point_count; ++i) {
      first[i] = full_density(fit[0], (*samples)[i].wi) / first_mass;
      second[i] = full_density(fit[1], (*samples)[i].wi) / second_mass;
...
uint16_t half(double value) {
  ...
  if (exponent < -24) return static_cast<uint16_t>(sign << 15u);
  if (exponent < -14) {
    mantissa |= 0x800000u;
...
int main(int argc, char **argv) {
  ...
  for (uint32_t y = 0; y < VKR_SHEEN_LTC_LUT_SIZE; ++y) {
    ...
    const double peak = peak_energy(roughness);
  ...
  for (uint32_t y = 0; y < VKR_SHEEN_ENERGY_LUT_SIZE; ++y) {
    ...
    for (uint32_t x = 0; x < VKR_SHEEN_ENERGY_LUT_SIZE; ++x) {
      ...
      const double value = directional_energy(no_v, roughness) * scale;
  ...
  std::array<std::array<FullFit, 2>, VKR_SHEEN_LTC_LUT_SIZE> previous_row = {};
  for (uint32_t y = 0; y < VKR_SHEEN_LTC_LUT_SIZE; ++y) {
    std::array<FullFit, 2> left = {};
    bool has_left = false;
    ...
    for (uint32_t x_index = 0; x_index < VKR_SHEEN_LTC_LUT_SIZE; ++x_index) {
      ...
      std::vector<double> residuals;
      const std::array<FullFit, 2> upper = previous_row[x];
      if (y == 0u) {
      ...
      for (const FullFit &component : pair) {
        if (!std::isfinite(component.log_s) || !std::isfinite(component.log_k) ||
          ...
          std::cerr << "invalid encoded Charlie LTC at " << x << ',' << y << '\n';
          return 1;
        }
      }
      if (std::abs(pair[0].fraction + pair[1].fraction - 1.0) > 1.0e-9) {
        std::cerr << "invalid Charlie LTC mixture at " << x << ',' << y << '\n';
        return 1;
      }
      left = pair;
      has_left = true;
  ...
  const std::string existing((std::istreambuf_iterator<char>(previous_file)), {});
