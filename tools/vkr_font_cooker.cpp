...
// Identity hashing uses the shared FIPS 180-4 implementation; finalizing a copy
// leaves the running state usable for further updates.
void sha_update(VkrSha256 &sha, const void *data, size_t length) {
  vkr_sha256_update(&sha, data, length);
}

std::array<uint8_t, 32> sha_final(VkrSha256 sha) {
  std::array<uint8_t, 32> result{};
  vkr_sha256_final(&sha, result.data());
  return result;
}

void hash_u32(VkrSha256 &sha, uint32_t value) {
  uint8_t b[4] = {uint8_t(value), uint8_t(value >> 8u), uint8_t(value >> 16u),
                  uint8_t(value >> 24u)};
  sha_update(sha, b, 4);
}
void hash_u64(VkrSha256 &sha, uint64_t value) {
  hash_u32(sha, uint32_t(value));
  hash_u32(sha, uint32_t(value >> 32u));
}
void hash_double(VkrSha256 &sha, double value) {
  uint64_t bits;
  std::memcpy(&bits, &value, sizeof(bits));
  hash_u64(sha, bits);
}
void hash_string(VkrSha256 &sha, const std::string &value) {
...
void add_identity(VkrSha256 &sha, const Config &config,
