...
// Access Chains mostly have their Base ID pointed directly to a OpVariable, but
// sometimes it will be through a load and this funciton handles the edge cases
// how to find that
static uint32_t
FindAccessChainBaseVariable(SpvReflectPrvParser *p_parser,
  ...
  while (base_node->op != SpvOpVariable) {
    ...
    case SpvOpBitcast:
      // This can be caused by something like GL_EXT_buffer_reference_uvec2
      // trying to load a pointer. We currently call from a push constant, so no
      // way to have a reference loop back into the PC block
      return 0;
    default: {
      assert(false);
    } break;
...
static SpvReflectResult ParseFunction(SpvReflectPrvParser *p_parser,
  ...
  if (p_func->parameter_count > 0) {
    p_func->parameters = (uint32_t *)calloc(p_func->parameter_count,
  ...
  for (size_t i = first_label_index; i < p_parser->node_count; ++i) {
    ...
    case SpvOpFunctionCall: {
      ...
      for (uint32_t j = 0, parameter_count = p_node->word_count - 4;
           j < parameter_count; j++) {
...
ParseDescriptorBindings(SpvReflectPrvParser *p_parser,
  ...
  for (size_t i = 0; i < p_parser->node_count; ++i) {
    ...
    if (p_type->op == SpvOpTypePointer) {
      assert(p_type->storage_class != -1 &&
             "Pointer types must have a valid storage class.");
  ...
  if (IsNotNull(p_type->members) && (p_type->member_count > 0)) {
    ...
    for (uint32_t member_index = 0; member_index < p_type->member_count;
      ...
      if ((p_member_type->storage_class ==
        ...
        if (!found_recursion) {
          SpvReflectTypeDescription *struct_type =
              FindType(p_module, p_member_type->id);
...
static SpvReflectResult ParseDescriptorBlockVariableSizes(
  ...
  for (uint32_t member_index = 0; member_index < p_var->member_count;
    ...
    case SpvOpTypePointer: {
      ...
      // If we found a struct, we need to fall through and get the size of it or
      // else we grab the size here
  ...
  // Structs can offset order don't need to match the index order, so first
  // order by offset example:
  ...
  // entry with largest offset value
  SpvReflectBlockVariable *p_last_member_var =
      pp_member_offset_order[p_var->member_count - 1];
  p_last_member_var->padded_size =
      RoundUp(p_last_member_var->offset + p_last_member_var->size,
...
static bool HasByteAddressBufferOffset(SpvReflectPrvNode *p_node,
  ...
  return IsNotNull(p_node) && IsNotNull(p_binding) &&
         (p_node->op == SpvOpAccessChain ||
          p_node->op == SpvOpInBoundsAccessChain) &&
         p_node->word_count == 6 &&
...
static SpvReflectResult ParseFunctionParameterAccess(
    SpvReflectPrvParser *p_parser, uint32_t callee_function_id,
  ...
  for (size_t i = 0; i < p_func->accessed_variable_count; ++i) {
    if (p_func->parameters[function_parameter_index] ==
      ...
      if (p_var->function_id > 0) {
        SpvReflectResult result = ParseFunctionParameterAccess(
            p_parser, p_var->function_id, p_var->function_parameter_index,
            p_accessed);
        if (result != SPV_REFLECT_RESULT_SUCCESS) {
          return result;
        }
...
static SpvReflectResult ParseStaticallyUsedResources(
  ...
  // If there are no used accessed, this is something like an empty
  // function/early return Basically there is going to be nothing to reflect,
  // but everything after this expects |p_used_accesses| to be allocated with
  ...
  SpvReflectPrvAccessedVariable *p_used_accesses =
      (SpvReflectPrvAccessedVariable *)calloc(
      ...
      if (p_var->variable_ptr == p_binding->spirv_id) {
        if (p_var->function_id > 0) {
          result = ParseFunctionParameterAccess(p_parser, p_var->function_id,
                                                p_var->function_parameter_index,
                                                &p_binding->accessed);
          if (result != SPV_REFLECT_RESULT_SUCCESS) {
            SafeFree(p_used_accesses);
            return result;
  ...
  for (uint32_t i = 0; i < p_parser->node_count; ++i) {
    ...
    if (p_node->op == SpvOpDecorate) {
      ...
      if (decoration == SpvDecorationSpecId) {
        ...
        CHECKED_READU32(p_parser, p_node->word_offset + 1,
                        p_spec_constant->spirv_id);
        CHECKED_READU32(p_parser, p_node->word_offset + 3,
                        p_spec_constant->constant_id);
        SpvReflectPrvNode *target_node =
            FindNode(p_parser, p_spec_constant->spirv_id);
        if (IsNotNull(target_node)) {
          // If being used for a OpSpecConstantComposite (ex. LocalSizeId),
