...
#define VectorConstructor(type, name)                                          \
  ...
  static inline VKR_MAYBE_UNUSED VKR_MUST_USE                                  \
      Vector_##name vector_create_##name##_with_capacity(                      \
  ...
  static inline VKR_MAYBE_UNUSED VKR_MUST_USE                                  \
      Vector_##name vector_create_##name(VkrAllocator *allocator) {            \
    return vector_create_##name##_with_capacity(allocator,                     \
                                                DEFAULT_VECTOR_CAPACITY);      \
  }                                                                            \
  static inline VKR_MAYBE_UNUSED VKR_MUST_USE bool8_t vector_reserve_##name(   \
  ...
  static inline VKR_MAYBE_UNUSED VKR_MUST_USE bool8_t vector_resize_##name(    \
      Vector_##name *vector) {                                                 \
  ...
  static inline VKR_MAYBE_UNUSED VKR_MUST_USE bool8_t vector_push_##name(      \
      Vector_##name *vector, type value) {                                     \
  ...
  static inline VKR_MAYBE_UNUSED type vector_pop_##name(                       \
      Vector_##name *vector) {                                                 \
    assert_log(vector != NULL, "Vector is NULL");                              \
    assert_log(vector->allocator != NULL, "Allocator is NULL");                \
  ...
  static inline VKR_MAYBE_UNUSED type *vector_pop_at_##name(                   \
      Vector_##name *vector, uint64_t index, type *dest) {                     \
    assert_log(vector != NULL, "Vector is NULL");                              \
    assert_log(vector->allocator != NULL, "Allocator is NULL");                \
  ...
  typedef bool8_t (*VectorFindCallback_##name)(type * current_value,           \
                                               type * value);                  \
  static inline VKR_MAYBE_UNUSED VectorFindResult vector_find_##name(          \
  ...
  static inline VKR_MAYBE_UNUSED void vector_clear_##name(                     \
      Vector_##name *vector) {                                                 \
    assert_log(vector != NULL, "Vector is NULL");                              \
    assert_log(vector->allocator != NULL, "Allocator is NULL");                \
    vector->length = 0;                                                        \
  }                                                                            \
  static inline VKR_MAYBE_UNUSED void vector_set_##name(                       \
      Vector_##name *vector, uint64_t index, type value) {                     \
    assert_log(vector != NULL, "Vector is NULL");                              \
    assert_log(vector->allocator != NULL, "Allocator is NULL");                \
    assert_log(index < vector->length, "Index is out of bounds");              \
  ...
  static inline VKR_MAYBE_UNUSED type *vector_get_##name(                      \
      const Vector_##name *vector, uint64_t index) {                           \
    assert_log(vector != NULL, "Vector is NULL");                              \
    assert_log(vector->allocator != NULL, "Allocator is NULL");                \
    assert_log(index < vector->length, "Index is out of bounds");              \
