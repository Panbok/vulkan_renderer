...
typedef struct VkrBindlessVkAllocation {
  ...
  VkrGpuAllocationHandle suballocation;
  uint32_t pool_index;
  bool8_t pooled;
...
typedef struct VkrBindlessVkPublishedTexture {
  ...
  VkSampler sampler;
  VkrTextureDescription sampler_description;
  VkrGpuSlotHandle sampled_slot;
  VkrGpuSlotHandle storage_slot;
  VkrGpuSlotHandle sampler_slot;
...
struct VkrBindlessVulkanRenderer {
  ...
  VkrBindlessVkMemoryBlock upload_blocks[4];
  ...
  VkCommandBuffer asset_command_buffer;
...
static bool8_t vkr_bindless_vk_choose_memory_type(
    ...
    VkMemoryPropertyFlags required, const VkMemoryPropertyFlags *preferences,
    uint32_t preference_count, uint32_t *out_index,
  ...
  for (uint32_t preference = 0; preference < preference_count; ++preference) {
    ...
    for (uint32_t i = 0; i < memory->memoryTypeCount; ++i) {
      ...
      if ((memory_type_bits & (1u << i)) && (available & wanted) == wanted) {
        *out_index = i;
        *out_properties = available;
        return true_v;
      }
    }
  }
  for (uint32_t i = 0; i < memory->memoryTypeCount; ++i) {
    const VkMemoryPropertyFlags available =
        memory->memoryTypes[i].propertyFlags;
    if ((memory_type_bits & (1u << i)) && (available & required) == required) {
      *out_index = i;
      *out_properties = available;
      return true_v;
    }
...
static void vkr_bindless_vk_destroy_buffer(VkrBindlessVulkanRenderer *renderer,
  ...
  if (buffer->allocation.mapped && !buffer->allocation.pooled) {
    vkUnmapMemory(device, buffer->allocation.memory);
  }
  if (buffer->handle) {
    vkDestroyBuffer(device, buffer->handle, NULL);
  }
  VkrBindlessVkMemoryBlock *pool =
      buffer->allocation.pool_index < renderer->upload_block_count
          ? &renderer->upload_blocks[buffer->allocation.pool_index]
          : NULL;
  if (buffer->allocation.pooled && pool && pool->core) {
    if (vkr_gpu_memory_retire(pool->core, buffer->allocation.suballocation,
                              renderer->completed_value) ==
        VKR_GPU_MEMORY_STATUS_OK)
      (void)vkr_gpu_memory_collect(pool->core, renderer->completed_value, NULL,
                                   NULL, NULL);
  } else if (buffer->allocation.memory) {
    vkFreeMemory(device, buffer->allocation.memory, NULL);
  }
  MemZero(buffer, sizeof(*buffer));
}

static bool8_t vkr_bindless_vk_create_buffer(
    VkrBindlessVulkanRenderer *renderer, VkDeviceSize size,
    VkBufferUsageFlags usage, VkMemoryPropertyFlags required_memory,
    const VkMemoryPropertyFlags *preferences, uint32_t preference_count,
    bool8_t mapped, VkrBindlessVkBuffer *out_buffer) {
  ...
  vkGetBufferMemoryRequirements(device, out_buffer->handle, &requirements);
  VkrBindlessVkAllocation *allocation = &out_buffer->allocation;
  if (!vkr_bindless_vk_choose_memory_type(
          renderer, requirements.memoryTypeBits, required_memory, preferences,
          preference_count, &allocation->memory_type_index,
  ...
  allocation->memory_size = requirements.size;
  if (vkAllocateMemory(device, &allocate_info, NULL, &allocation->memory) !=
          VK_SUCCESS ||
      vkBindBufferMemory(device, out_buffer->handle, allocation->memory, 0u) !=
          VK_SUCCESS ||
      (mapped &&
...
typedef struct VkrBindlessVkPooledBufferSpec {
  VkrBindlessVkBuffer *buffer;
  VkDeviceSize size;
  VkBufferUsageFlags usage;
} VkrBindlessVkPooledBufferSpec;

static bool8_t
vkr_bindless_vk_create_upload_block(VkrBindlessVulkanRenderer *renderer) {
  ...
  VkrBindlessVkPooledBufferSpec specs[] = {
      {&renderer->resource_descriptors, resource_layout->size,
       VK_BUFFER_USAGE_RESOURCE_DESCRIPTOR_BUFFER_BIT_EXT |
           VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT},
      {&renderer->sampler_descriptors, sampler_layout->size,
       VK_BUFFER_USAGE_SAMPLER_DESCRIPTOR_BUFFER_BIT_EXT |
           VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT},
      {&renderer->upload, VKR_BINDLESS_VK_UPLOAD_SIZE,
       VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT |
           VK_BUFFER_USAGE_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT},
      {&renderer->materials,
       (VkDeviceSize)renderer->config.material_capacity *
           sizeof(VkrBindlessVkMaterialGpuRow),
       VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT},
  };
  VkMemoryRequirements requirements[ArrayCount(specs)];
  VkrGpuAllocationHandle handles[ArrayCount(specs)];
  VkrGpuPlacement placements[ArrayCount(specs)];
  uint32_t pool_indices[ArrayCount(specs)];
  MemZero(requirements, sizeof(requirements));
  MemZero(handles, sizeof(handles));
  MemZero(placements, sizeof(placements));
  MemZero(pool_indices, sizeof(pool_indices));
  ...
  renderer->upload_block_count = 0u;
  for (uint32_t i = 0; i < ArrayCount(specs); ++i) {
    VkrBindlessVkBuffer *buffer = specs[i].buffer;
    MemZero(buffer, sizeof(*buffer));
    buffer->size = specs[i].size;
    ...
    if (vkCreateBuffer(device, &buffer_info, NULL, &buffer->handle) !=
        VK_SUCCESS) {
      log_error("Bindless Vulkan upload block failed to create buffer %u", i);
      return false_v;
    }
    vkGetBufferMemoryRequirements(device, buffer->handle, &requirements[i]);
    uint32_t memory_type_index = 0u;
    VkMemoryPropertyFlags properties = 0u;
    if (!vkr_bindless_vk_choose_memory_type(
            renderer, requirements[i].memoryTypeBits,
            VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, preferences,
            ArrayCount(preferences), &memory_type_index, &properties)) {
      log_error("Bindless Vulkan upload buffer %u has no memory type", i);
      return false_v;
    }
    uint32_t pool_index = UINT32_MAX;
    for (uint32_t pool = 0; pool < renderer->upload_block_count; ++pool) {
      if (renderer->upload_blocks[pool].memory_type_index ==
          memory_type_index) {
        pool_index = pool;
        break;
      }
    ...
    if (pool_index == UINT32_MAX) {
      if (renderer->upload_block_count == ArrayCount(renderer->upload_blocks)) {
        log_error("Bindless Vulkan upload block table is full");
        return false_v;
      }
      pool_index = renderer->upload_block_count++;
      renderer->upload_blocks[pool_index].memory_type_index = memory_type_index;
      renderer->upload_blocks[pool_index].properties = properties;
    }
    pool_indices[i] = pool_index;
    VkrBindlessVkMemoryBlock *block = &renderer->upload_blocks[pool_index];
    if (block->size > UINT64_MAX - (requirements[i].alignment - 1u)) {
      log_error("Bindless Vulkan upload block size overflow");
      return false_v;
    }
    block->size = AlignPow2(block->size, requirements[i].alignment);
    if (requirements[i].size > UINT64_MAX - block->size) {
      log_error("Bindless Vulkan upload block requirement overflow");
      return false_v;
    }
    block->size += requirements[i].size;
  }

  for (uint32_t pool_index = 0; pool_index < renderer->upload_block_count;
       ++pool_index) {
    VkrBindlessVkMemoryBlock *block = &renderer->upload_blocks[pool_index];
    uint32_t allocation_count = 0u;
    for (uint32_t i = 0; i < ArrayCount(specs); ++i)
      allocation_count += pool_indices[i] == pool_index;
    const VkrGpuMemoryConfig core_config = {
        .heap_size = block->size,
        .max_allocations = allocation_count,
        .max_retirements = allocation_count,
        .max_free_ranges = allocation_count + 1u,
    };
    block->core_storage_size = vkr_gpu_memory_storage_requirement(&core_config);
    block->core_storage =
        vkr_allocator_alloc(renderer->allocator, block->core_storage_size,
                            VKR_ALLOCATOR_MEMORY_TAG_RENDERER);
    if (!block->core_storage ||
        vkr_gpu_memory_create(&core_config, block->core_storage,
                              block->core_storage_size,
                              &block->core) != VKR_GPU_MEMORY_STATUS_OK) {
      log_error("Bindless Vulkan upload allocator core %u creation failed",
                pool_index);
      return false_v;
    }
  }
  for (uint32_t i = 0; i < ArrayCount(specs); ++i) {
    VkrBindlessVkMemoryBlock *block = &renderer->upload_blocks[pool_indices[i]];
    if (vkr_gpu_memory_allocate(block->core, requirements[i].size,
                                requirements[i].alignment,
                                VKR_GPU_MEMORY_CLASS_BUFFER, &handles[i],
                                &placements[i]) != VKR_GPU_MEMORY_STATUS_OK) {
      log_error("Bindless Vulkan upload suballocation %u failed", i);
      return false_v;
    }
  }
  ...
  for (uint32_t pool_index = 0; pool_index < renderer->upload_block_count;
    ...
    if (vkAllocateMemory(device, &allocate_info, NULL, &block->memory) !=
            VK_SUCCESS ||
        vkMapMemory(device, block->memory, 0u, block->size, 0u,
                    &block->mapped) != VK_SUCCESS) {
      vkr_gpu_memory_record_native_failure(block->core);
      log_error("Bindless Vulkan upload block %u allocation or mapping failed",
                pool_index);
      return false_v;
    }
  }
  for (uint32_t i = 0; i < ArrayCount(specs); ++i) {
    VkrBindlessVkBuffer *buffer = specs[i].buffer;
    VkrBindlessVkMemoryBlock *block = &renderer->upload_blocks[pool_indices[i]];
    if (vkBindBufferMemory(device, buffer->handle, block->memory,
                           placements[i].resource_offset) != VK_SUCCESS) {
      vkr_gpu_memory_record_native_failure(block->core);
      log_error("Bindless Vulkan upload buffer %u binding failed", i);
      return false_v;
    }
    buffer->allocation = (VkrBindlessVkAllocation){
        .memory = block->memory,
        .memory_size = block->size,
        .offset = placements[i].resource_offset,
        .mapped = (uint8_t *)block->mapped + placements[i].resource_offset,
        .memory_type_index = block->memory_type_index,
        .properties = block->properties,
        .suballocation = handles[i],
    ...
    if (!buffer->address) {
      log_error("Bindless Vulkan upload buffer %u has no device address", i);
      return false_v;
    }
  }
  return true_v;
...
static void
vkr_bindless_vk_destroy_upload_block(VkrBindlessVulkanRenderer *renderer) {
  VkDevice device = vkr_bindless_vk_renderer_device(renderer);
  for (uint32_t pool_index = 0; pool_index < renderer->upload_block_count;
       ++pool_index) {
    VkrBindlessVkMemoryBlock *block = &renderer->upload_blocks[pool_index];
    ...
    if (block->core_storage)
      vkr_allocator_free(renderer->allocator, block->core_storage,
                         block->core_storage_size,
                         VKR_ALLOCATOR_MEMORY_TAG_RENDERER);
...
static void vkr_bindless_vk_destroy_image(VkrBindlessVulkanRenderer *renderer,
  ...
  if (image->allocation.memory) {
    vkFreeMemory(device, image->allocation.memory, NULL);
  }
...
static bool8_t vkr_bindless_vk_create_image_ex(
  ...
  VkMemoryRequirements requirements;
  vkGetImageMemoryRequirements(device, out_image->handle, &requirements);
  const VkMemoryPropertyFlags preferences[] = {
      VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
  };
  VkrBindlessVkAllocation *allocation = &out_image->allocation;
  if (!vkr_bindless_vk_choose_memory_type(renderer, requirements.memoryTypeBits,
                                          VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
                                          preferences, ArrayCount(preferences),
  ...
  if (vkAllocateMemory(device, &allocate_info, NULL, &allocation->memory) !=
          VK_SUCCESS ||
      vkBindImageMemory(device, out_image->handle, allocation->memory, 0u) !=
          VK_SUCCESS) {
    vkr_bindless_vk_destroy_image(renderer, out_image);
    return false_v;
  }
  ...
  for (uint32_t i = 0; i < VKR_BINDLESS_VK_FRAME_SLOT_COUNT; ++i) {
    ...
    if (vkAllocateCommandBuffers(device, &command_info,
        ...
        !vkr_bindless_vk_create_buffer(
            renderer, readback_size, VK_BUFFER_USAGE_TRANSFER_DST_BIT,
            VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, readback_preferences,
            ArrayCount(readback_preferences), true_v, &slot->readback)) {
      return false_v;
    }
  }
  return true_v;
...
static bool8_t
vkr_bindless_vk_create_asset_commands(VkrBindlessVulkanRenderer *renderer) {
  VkDevice device = vkr_bindless_vk_renderer_device(renderer);
  ...
  if (vkCreateCommandPool(device, &pool_info, NULL,
                          &renderer->asset_command_pool) != VK_SUCCESS)
    return false_v;
  const VkCommandBufferAllocateInfo command_info = {
      .sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO,
      .commandPool = renderer->asset_command_pool,
      .level = VK_COMMAND_BUFFER_LEVEL_PRIMARY,
      .commandBufferCount = 1u,
  };
  return vkAllocateCommandBuffers(device, &command_info,
                                  &renderer->asset_command_buffer) ==
         VK_SUCCESS;
}
...
vkr_bindless_vk_create_resources(VkrBindlessVulkanRenderer *renderer) {
  if (!vkr_bindless_vk_create_upload_block(renderer) ||
      ...
      !vkr_bindless_vk_create_asset_commands(renderer)) {
    return false_v;
  }
...
static bool8_t vkr_bindless_vk_create_descriptor_slot_tables(
  ...
  const VkrGpuSlotTableConfig material_config = {
      .max_slots = renderer->config.material_capacity,
      .max_retirements = renderer->config.material_capacity,
  ...
  renderer->published_materials_size =
      (uint64_t)renderer->config.material_capacity *
  ...
  if (!renderer->sampled_image_slot_storage ||
      ...
      !renderer->published_textures || !renderer->published_materials ||
      !renderer->retired_materials) {
    return false_v;
  }
...
static bool8_t vkr_bindless_vk_publish_sentinel_descriptors(
  ...
  return vkr_bindless_vk_flush(renderer,
                               &renderer->resource_descriptors.allocation,
                               resource_layout->sampled_image_offset,
                               properties->sampledImageDescriptorSize) &&
         vkr_bindless_vk_flush(renderer,
                               &renderer->resource_descriptors.allocation,
                               resource_layout->storage_image_offset,
                               properties->storageImageDescriptorSize) &&
         vkr_bindless_vk_flush(renderer,
                               &renderer->sampler_descriptors.allocation,
                               sampler_layout->sampler_offset,
                               properties->samplerDescriptorSize) &&
         vkr_bindless_vk_flush(renderer, &renderer->materials.allocation, 0u,
                               sizeof(material));
}
...
bool8_t vkr_bindless_vulkan_renderer_create(
  ...
  if (!config || !config->allocator || !out_renderer || !config->width ||
      !config->height || !config->image_count ||
      config->image_count > VKR_BINDLESS_VK_TARGET_IMAGE_MAX ||
      !config->material_capacity) {
    return false_v;
  }
  *out_renderer = NULL;
...
static bool8_t
vkr_bindless_vk_upload_prepared_texture(VkrBindlessVulkanRenderer *renderer,
                                        const VkrTexturePreparedLoad *prepared,
                                        VkrBindlessVkImage *out_image) {
  if (!prepared || !out_image || !prepared->upload_data ||
      !prepared->upload_data_size || !prepared->upload_regions ||
      !prepared->upload_region_count || !prepared->upload_mip_levels ||
  ...
  const VkMemoryPropertyFlags staging_preferences[] = {
      VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
      0u,
  };
  ...
  if (!vkr_bindless_vk_create_buffer(
          ...
          staging_preferences, ArrayCount(staging_preferences), true_v,
          &staging)) {
    vkr_bindless_vk_destroy_image(renderer, out_image);
    return false_v;
  }
  ...
  VkDevice device = vkr_bindless_vk_renderer_device(renderer);
  if (valid)
    valid = vkResetCommandPool(device, renderer->asset_command_pool, 0u) ==
            VK_SUCCESS;
  ...
  if (valid)
    valid = vkBeginCommandBuffer(renderer->asset_command_buffer, &begin_info) ==
            VK_SUCCESS;
  if (valid) {
    vkr_bindless_vk_cmd_image_barrier_range(
        renderer->asset_command_buffer, out_image->handle,
    ...
    const VkCopyBufferToImageInfo2 copy_info = {
        ...
        .dstImage = out_image->handle,
        .dstImageLayout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
        .regionCount = prepared->upload_region_count,
        .pRegions = regions,
    };
    vkCmdCopyBufferToImage2(renderer->asset_command_buffer, &copy_info);
    vkr_bindless_vk_cmd_image_barrier_range(
        renderer->asset_command_buffer, out_image->handle,
        VK_PIPELINE_STAGE_2_COPY_BIT, VK_ACCESS_2_TRANSFER_WRITE_BIT,
        ...
        VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, prepared->upload_mip_levels,
        prepared->upload_array_layers);
    valid = vkEndCommandBuffer(renderer->asset_command_buffer) == VK_SUCCESS;
  }
  const uint64_t signal_value = renderer->submit_value + 1u;
  const VkCommandBufferSubmitInfo command_info = {
      .sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_SUBMIT_INFO,
      .commandBuffer = renderer->asset_command_buffer,
  };
  ...
  if (valid)
    valid = vkQueueSubmit2(vkr_bindless_vulkan_device_queue(renderer->device),
                           1u, &submit_info, VK_NULL_HANDLE) == VK_SUCCESS;
  if (valid) {
    ...
    valid = vkWaitSemaphores(device, &wait_info, UINT64_MAX) == VK_SUCCESS;
    if (valid)
      renderer->completed_value = signal_value;
  }
  out_image->layout = valid ? VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL
                            : VK_IMAGE_LAYOUT_UNDEFINED;
  vkr_allocator_free(renderer->allocator, regions, regions_size,
                     VKR_ALLOCATOR_MEMORY_TAG_RENDERER);
  vkr_bindless_vk_destroy_buffer(renderer, &staging);
  if (!valid)
    vkr_bindless_vk_destroy_image(renderer, out_image);
...
void vkr_bindless_vulkan_renderer_memory_metrics(
  ...
  if (!renderer)
    return;
  out_metrics->block_count = renderer->upload_block_count;
  for (uint32_t block_index = 0; block_index < renderer->upload_block_count;
       ++block_index) {
    vkr_gpu_memory_get_metrics(renderer->upload_blocks[block_index].core,
                               &out_metrics->blocks[block_index]);
...
static bool8_t
vkr_bindless_vk_publish_sampled_view(VkrBindlessVulkanRenderer *renderer,
                                     VkImageView view,
  ...
  if (vkr_gpu_slot_table_publish(renderer->sampled_image_slots,
    ...
    return false_v;
  return vkr_bindless_vk_flush(
      renderer, &renderer->resource_descriptors.allocation,
...
vkr_bindless_vk_publish_sampler(VkrBindlessVulkanRenderer *renderer,
  ...
  if (vkr_gpu_slot_table_publish(renderer->sampler_slots,
    ...
    return false_v;
  return vkr_bindless_vk_flush(
      renderer, &renderer->sampler_descriptors.allocation,
      layout->sampler_offset +
          (VkDeviceSize)out_handle->index * properties->samplerDescriptorSize,
      properties->samplerDescriptorSize);
}

static bool8_t vkr_bindless_vk_replace_sampler(
    VkrBindlessVulkanRenderer *renderer, VkrGpuSlotHandle old_handle,
    uint64_t old_last_use_submit_value, VkSampler sampler,
    VkrGpuSlotHandle *out_handle) {
  const VkPhysicalDeviceDescriptorBufferPropertiesEXT *properties =
      vkr_bindless_vulkan_device_descriptor_properties(renderer->device);
  const VkrBindlessVulkanDescriptorLayout *layout =
  ...
  vkr_bindless_vulkan_device_get_descriptor(renderer->device)(
      vkr_bindless_vk_renderer_device(renderer), &get_info,
      properties->samplerDescriptorSize, renderer->descriptor_scratch);
  if (vkr_gpu_slot_table_replace(
          renderer->sampler_slots, old_handle, renderer->descriptor_scratch,
          old_last_use_submit_value, out_handle) != VKR_GPU_SLOT_STATUS_OK)
    return false_v;
  return vkr_bindless_vk_flush(
      renderer, &renderer->sampler_descriptors.allocation,
      layout->sampler_offset +
...
vkr_bindless_vk_publish_material_gpu_row(VkrBindlessVulkanRenderer *renderer,
  ...
  if (vkr_gpu_slot_table_publish(renderer->material_slots, row, out_handle) !=
    ...
    return false_v;
  return vkr_bindless_vk_flush(renderer, &renderer->materials.allocation,
...
static bool8_t
vkr_bindless_vk_sampler_description_equal(const VkrTextureDescription *a,
                                          const VkrTextureDescription *b) {
  return a && b && a->min_filter == b->min_filter &&
         a->mag_filter == b->mag_filter && a->mip_filter == b->mip_filter &&
...
static bool8_t
vkr_bindless_vk_transition_writable_image(VkrBindlessVulkanRenderer *renderer,
                                          VkrBindlessVkImage *image) {
  VkDevice device = vkr_bindless_vk_renderer_device(renderer);
  if (vkResetCommandPool(device, renderer->asset_command_pool, 0u) !=
      VK_SUCCESS)
    return false_v;
  const VkCommandBufferBeginInfo begin_info = {
      .sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO,
  ...
  if (vkBeginCommandBuffer(renderer->asset_command_buffer, &begin_info) !=
      VK_SUCCESS)
    return false_v;
  vkr_bindless_vk_cmd_image_barrier_range(
      renderer->asset_command_buffer, image->handle, VK_PIPELINE_STAGE_2_NONE,
      VK_ACCESS_2_NONE,
      VK_PIPELINE_STAGE_2_FRAGMENT_SHADER_BIT |
      ...
      VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL, image->mip_levels,
      image->array_layers);
  if (vkEndCommandBuffer(renderer->asset_command_buffer) != VK_SUCCESS)
    return false_v;
  const uint64_t signal_value = renderer->submit_value + 1u;
  const VkCommandBufferSubmitInfo command_info = {
      .sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_SUBMIT_INFO,
  ...
  renderer->upload_wait_count++;
  if (vkWaitSemaphores(device, &wait_info, UINT64_MAX) != VK_SUCCESS)
    return false_v;
  renderer->completed_value = signal_value;
...
static bool8_t vkr_bindless_vk_retire_unreferenced_texture(
  ...
  if (vkr_gpu_slot_table_retire(renderer->sampled_image_slots,
      ...
      (texture->has_storage_slot &&
       vkr_gpu_slot_table_retire(renderer->storage_image_slots,
                                 texture->storage_slot,
                                 completed) != VKR_GPU_SLOT_STATUS_OK) ||
      vkr_gpu_slot_table_retire(renderer->sampler_slots, texture->sampler_slot,
                                completed) != VKR_GPU_SLOT_STATUS_OK ||
  ...
  if (texture->sampler)
    vkDestroySampler(vkr_bindless_vk_renderer_device(renderer),
...
static bool8_t vkr_bindless_vk_create_published_buffer(
    ...
    VkBufferUsageFlags usage, VkrBindlessVkBuffer *out_buffer) {
  if (!data || !size)
    return false_v;
  const VkMemoryPropertyFlags preferences[] = {
      VK_MEMORY_PROPERTY_HOST_COHERENT_BIT |
  ...
  if (!vkr_bindless_vk_create_buffer(
          renderer, size, usage, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT,
          preferences, ArrayCount(preferences), true_v, out_buffer))
    return false_v;
  MemCopy(out_buffer->allocation.mapped, data, size);
  if (!vkr_bindless_vk_flush(renderer, &out_buffer->allocation, 0u, size)) {
    vkr_bindless_vk_destroy_buffer(renderer, out_buffer);
    return false_v;
  }
  return true_v;
...
static bool8_t vkr_bindless_vk_asset_publish_writable_texture(
  ...
  const bool8_t image_created = vkr_bindless_vk_create_image_ex(
      ...
      &pending.image);
  const bool8_t transitioned =
      image_created &&
      vkr_bindless_vk_transition_writable_image(renderer, &pending.image);
  const bool8_t sampler_created =
      transitioned && vkr_bindless_vk_create_published_sampler(
                          renderer, description, mip_levels, &pending.sampler);
  const bool8_t sampled_published =
      sampler_created &&
      vkr_bindless_vk_publish_sampled_view(renderer, pending.image.view,
  ...
  const bool8_t sampler_published =
      storage_published &&
      vkr_bindless_vk_publish_sampler(renderer, pending.sampler,
                                      &pending.sampler_slot);
  if (!sampler_published) {
    VkrGpuSlotTable *tables[] = {renderer->sampled_image_slots,
                                 renderer->storage_image_slots,
                                 renderer->sampler_slots};
    const VkrGpuSlotHandle handles[] = {
        pending.sampled_slot, pending.storage_slot, pending.sampler_slot};
...
static bool8_t vkr_bindless_vk_asset_update_texture_sampler(
  ...
  if (!texture || !description || renderer->frame_active ||
      ...
      texture->material_reference_count != 0u)
    return false_v;
  if (vkr_bindless_vk_sampler_description_equal(&texture->sampler_description,
                                                description))
    return true_v;
  VkSampler replacement_sampler = VK_NULL_HANDLE;
  VkrGpuSlotHandle replacement_slot = {0};
  ...
  if (!vkr_bindless_vk_create_published_sampler(renderer, description,
                                                ...
                                                &replacement_sampler) ||
      !vkr_bindless_vk_replace_sampler(renderer, texture->sampler_slot,
                                       retire_value, replacement_sampler,
                                       &replacement_slot)) {
  ...
  const VkSampler old_sampler = texture->sampler;
  texture->sampler = replacement_sampler;
  texture->sampler_slot = replacement_slot;
  texture->sampler_description = *description;
  if (retire_value > vkr_bindless_vk_refresh_completed(renderer)) {
    const VkSemaphoreWaitInfo wait_info = {
        .sType = VK_STRUCTURE_TYPE_SEMAPHORE_WAIT_INFO,
        .semaphoreCount = 1u,
        .pSemaphores = &renderer->timeline,
        .pValues = &retire_value,
    };
    renderer->upload_wait_count++;
    if (vkWaitSemaphores(vkr_bindless_vk_renderer_device(renderer), &wait_info,
                         UINT64_MAX) != VK_SUCCESS)
      return false_v;
    renderer->completed_value = retire_value;
  }
  if (vkr_gpu_slot_table_collect(renderer->sampler_slots,
                                 renderer->completed_value,
                                 NULL) != VKR_GPU_SLOT_STATUS_OK)
    return false_v;
  vkDestroySampler(vkr_bindless_vk_renderer_device(renderer), old_sampler,
                   NULL);
  return true_v;
}
...
vkr_bindless_vk_asset_publish_texture(void *state, VkrTextureHandle handle,
  ...
  const bool8_t image_uploaded = vkr_bindless_vk_upload_prepared_texture(
      renderer, prepared, &pending.image);
  const bool8_t sampler_created =
      image_uploaded && vkr_bindless_vk_create_published_sampler(
                            renderer, &prepared->description,
                            prepared->upload_mip_levels, &pending.sampler);
  const bool8_t sampled_published =
      sampler_created &&
      vkr_bindless_vk_publish_sampled_view(renderer, pending.image.view,
                                           &pending.sampled_slot);
  ...
  if (!sampler_published) {
    log_error("Bindless Vulkan texture %u:%u publication failed at %s",
              ...
              : !sampler_created   ? "sampler creation"
    ...
    if (pending.sampler_slot.generation) {
      (void)vkr_gpu_slot_table_retire(renderer->sampler_slots,
                                      pending.sampler_slot,
                                      renderer->completed_value);
      (void)vkr_gpu_slot_table_collect(renderer->sampler_slots,
                                       renderer->completed_value, NULL);
    }
    if (pending.sampler)
      vkDestroySampler(vkr_bindless_vk_renderer_device(renderer),
  ...
  pending.sampler_description = prepared->description;
  pending.live = true_v;
  *record = pending;
  return true_v;
}
...
vkr_bindless_vk_asset_publish_material(void *state, VkrMaterialHandle handle,
  ...
  if (!renderer || !material || handle.id == 0u ||
      handle.id > renderer->config.material_capacity)
    return false_v;
  ...
  uint32_t texture_record_indices[4] = {0};
  ...
  for (uint32_t i = 0; i < ArrayCount(row_slots); ++i) {
    ...
    VkrBindlessVkPublishedTexture *texture =
        source->enabled ? vkr_bindless_vk_published_texture(
                              renderer, source->handle, &record_index)
                        : NULL;
    if (!texture)
      continue;
    texture_indices[i] = texture->sampled_slot.index;
    sampler_indices[i] = texture->sampler_slot.index;
    texture_record_indices[texture_count++] = record_index;
  }
  ...
  const VkrBindlessVkMaterialGpuRow row = {
      ...
      .flags = material->double_sided ? 1u : 0u,
  };
...
static bool8_t vkr_bindless_vk_run_asset_publisher_fixture(
  ...
  const VkrMaterialHandle material_a = {
      .id = renderer->config.material_capacity - 1u, .generation = 1u};
  const VkrMaterialHandle material_b = {
      .id = renderer->config.material_capacity, .generation = 1u};
  ...
  if (sampler_after_duplicate.slots_published !=
      ...
      !vkr_bindless_vk_asset_publish_material(renderer, material_a,
                                              &material) ||
      !vkr_bindless_vk_asset_publish_material(renderer, material_b, &material))
    return false_v;
  ...
  if (!vkr_bindless_vk_asset_publish_texture(renderer, replacement_texture,
                                             &prepared))
    return false_v;
  material.textures[VKR_TEXTURE_SLOT_DIFFUSE].handle = replacement_texture;
  if (!vkr_bindless_vk_asset_publish_material(renderer, material_b, &material))
    return false_v;
  renderer->active_material_index =
      renderer->published_materials[material_b.id - 1u].slot.index;
...
bool8_t vkr_bindless_vulkan_renderer_run_publication_test(
  ...
  if (!vkr_bindless_vk_publish_sampled_view(
          renderer, renderer->sentinel_image.view, &shared_texture) ||
  ...
  if (!vkr_bindless_vk_publish_sampled_view(
          renderer, renderer->sentinel_image.view, &replacement_texture) ||
      !vkr_bindless_vk_publish_storage_view(
...
void vkr_bindless_vulkan_renderer_destroy(VkrBindlessVulkanRenderer *renderer) {
  ...
  if (device) {
    ...
    if (renderer->asset_command_pool)
      vkDestroyCommandPool(device, renderer->asset_command_pool, NULL);
    ...
    vkr_bindless_vk_destroy_upload_block(renderer);
