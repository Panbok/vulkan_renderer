...
enum {
  ...
  VKR_VULKAN_KNOWN_VISIBILITY_FLAGS =
      VKR_GPU_VISIBILITY_DEVICE | VKR_GPU_VISIBILITY_RESOURCE_ALIAS,
  VKR_VULKAN_KNOWN_BUFFER_ACCESS_FLAGS =
      ...
      VKR_BUFFER_ACCESS_TRANSFER_DST | VKR_BUFFER_ACCESS_INDIRECT_READ,
  VKR_VULKAN_KNOWN_IMAGE_ACCESS_FLAGS =
...
vkr_vk_lower_stages(VkrGpuStageFlags stages,
                    VkPipelineStageFlags2 *out_stages) {
  if (!out_stages ||
      ((uint32_t)stages & ~(uint32_t)VKR_VULKAN_KNOWN_STAGE_FLAGS) != 0u) {
    return VKR_VULKAN_DEPENDENCY_INVALID_ARGUMENT;
  }
  ...
  *out_stages = lowered;
  return VKR_VULKAN_DEPENDENCY_OK;
}

vkr_internal VkrVulkanDependencyResult
vkr_vk_validate_dependency(const VkrGpuDependency *dependency) {
  if (((uint32_t)dependency->visibility &
       ~(uint32_t)VKR_VULKAN_KNOWN_VISIBILITY_FLAGS) != 0u) {
    return VKR_VULKAN_DEPENDENCY_INVALID_ARGUMENT;
  }
  if (dependency->visibility & VKR_GPU_VISIBILITY_RESOURCE_ALIAS) {
    return VKR_VULKAN_DEPENDENCY_RESOURCE_ALIAS_UNSUPPORTED;
  }
  return VKR_VULKAN_DEPENDENCY_OK;
}

vkr_internal VkAccessFlags2 vkr_vk_buffer_access(VkrBufferAccessFlags access) {
...
vkr_internal VkAccessFlags2 vkr_vk_image_access(VkrImageAccessFlags access,
                                                bool8_t is_src) {
  VkAccessFlags2 flags = VK_ACCESS_2_NONE;
...
VkrVulkanDependencyResult vkr_vk_lower_buffer_dependency(
  ...
  if (!out_lowered || (((uint32_t)src_access | (uint32_t)dst_access) &
                       ~(uint32_t)VKR_VULKAN_KNOWN_BUFFER_ACCESS_FLAGS) != 0u) {
    return VKR_VULKAN_DEPENDENCY_INVALID_ARGUMENT;
  }
  *out_lowered = (VkrVulkanDependency){0};
  ...
  VkrVulkanDependencyResult result = vkr_vk_validate_dependency(selected);
  if (result != VKR_VULKAN_DEPENDENCY_OK)
    return result;
  result = vkr_vk_lower_stages(selected->src_stages, &out_lowered->src_stages);
  if (result != VKR_VULKAN_DEPENDENCY_OK)
    return result;
  result = vkr_vk_lower_stages(selected->dst_stages, &out_lowered->dst_stages);
  if (result != VKR_VULKAN_DEPENDENCY_OK)
    return result;

  if (selected->visibility & VKR_GPU_VISIBILITY_DEVICE) {
    out_lowered->src_access = vkr_vk_buffer_access(src_access);
    out_lowered->dst_access = vkr_vk_buffer_access(dst_access);
  }
  return VKR_VULKAN_DEPENDENCY_OK;
}
...
VkrVulkanDependencyResult vkr_vk_lower_image_dependency(
  ...
  if (!out_lowered || (((uint32_t)src_access | (uint32_t)dst_access) &
                       ~(uint32_t)VKR_VULKAN_KNOWN_IMAGE_ACCESS_FLAGS) != 0u) {
    return VKR_VULKAN_DEPENDENCY_INVALID_ARGUMENT;
  }
  *out_lowered = (VkrVulkanDependency){0};
  ...
  if (selected->visibility & VKR_GPU_VISIBILITY_DEVICE) {
    out_lowered->src_access = vkr_vk_image_access(src_access, true_v);
    out_lowered->dst_access = vkr_vk_image_access(dst_access, false_v);
...
const char *vkr_vk_dependency_result_string(VkrVulkanDependencyResult result) {
  switch (result) {
  case VKR_VULKAN_DEPENDENCY_OK:
    return "ok";
  case VKR_VULKAN_DEPENDENCY_INVALID_ARGUMENT:
    return "invalid argument";
  case VKR_VULKAN_DEPENDENCY_RESOURCE_ALIAS_UNSUPPORTED:
    return "resource-alias visibility is unsupported without graph placement "
           "aliasing";
  default:
    return "unknown Vulkan dependency error";
  }
}
