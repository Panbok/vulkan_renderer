...
static void vkr_metal_capture_slot_reset(VkrMetalCaptureSlot *slot) {
  uint8_t *storage = slot->storage;
  MemZero(slot, sizeof(*slot));
  slot->storage = storage;
}

static VkrMetalCaptureSlot *
vkr_metal_capture_slot_find(VkrMetalCaptureRing *ring,
                            VkrCaptureRequestId request_id) {
  for (uint32_t i = 0; i < ring->capacity; ++i) {
    if (ring->slots[i].state != VKR_METAL_CAPTURE_SLOT_IDLE &&
...
uint64_t vkr_metal_capture_ring_storage_requirement(uint32_t capacity,
  ...
  if (capacity == 0 || capacity > VKR_METAL_CAPTURE_RING_CAPACITY_MAX ||
...
bool8_t vkr_metal_capture_ring_init(VkrMetalCaptureRing *ring,
  ...
  const uint64_t required =
      vkr_metal_capture_ring_storage_requirement(capacity, max_batch_bytes);
...
VkrRendererError vkr_metal_capture_ring_reserve(
  ...
  if (!ring || !ring->initialized || !request || request->request_id == 0 ||
      ...
      vkr_metal_capture_slot_find(ring, request->request_id)) {
  ...
  slot->state = VKR_METAL_CAPTURE_SLOT_RESERVED;
...
bool8_t vkr_metal_capture_ring_submit(VkrMetalCaptureRing *ring,
                                      ...
                                      const void *staging) {
  VkrMetalCaptureSlot *slot =
      ring ? vkr_metal_capture_slot_find(ring, request_id) : NULL;
  if (!slot || slot->state != VKR_METAL_CAPTURE_SLOT_RESERVED ||
      submit_serial == 0 || !staging) {
    return false_v;
  }
  slot->state = VKR_METAL_CAPTURE_SLOT_SUBMITTED;
...
bool8_t vkr_metal_capture_ring_fail(VkrMetalCaptureRing *ring,
  ...
  slot->state = VKR_METAL_CAPTURE_SLOT_FAILED;
  slot->error = error;
  return true_v;
}
...
void vkr_metal_capture_ring_collect(VkrMetalCaptureRing *ring,
  ...
  for (uint32_t i = 0; i < ring->capacity; ++i) {
    VkrMetalCaptureSlot *slot = &ring->slots[i];
    if ((slot->state != VKR_METAL_CAPTURE_SLOT_SUBMITTED &&
         slot->state != VKR_METAL_CAPTURE_SLOT_ABANDONED) ||
        slot->submit_serial == 0 ||
        slot->submit_serial > completed_submit_serial) {
      continue;
    }
    if (slot->state == VKR_METAL_CAPTURE_SLOT_ABANDONED) {
      vkr_metal_capture_slot_reset(slot);
      continue;
    }
    ...
    slot->staging = NULL;
    slot->state = VKR_METAL_CAPTURE_SLOT_READY;
  }
}
...
VkrCaptureStatus vkr_metal_capture_ring_poll(VkrMetalCaptureRing *ring,
  ...
  vkr_metal_capture_ring_collect(ring, completed_submit_serial);
  VkrMetalCaptureSlot *slot = vkr_metal_capture_slot_find(ring, request_id);
  if (!slot || slot->state == VKR_METAL_CAPTURE_SLOT_ABANDONED) {
    return VKR_CAPTURE_STATUS_NOT_FOUND;
  }
  ...
  if (slot->state == VKR_METAL_CAPTURE_SLOT_FAILED) {
    out_result->status = VKR_CAPTURE_STATUS_FAILED;
  } else if (slot->state == VKR_METAL_CAPTURE_SLOT_READY ||
             slot->state == VKR_METAL_CAPTURE_SLOT_ACQUIRED) {
    slot->state = VKR_METAL_CAPTURE_SLOT_ACQUIRED;
