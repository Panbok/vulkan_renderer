...
vkr_internal void vkr_pass_ui_draw_list(RendererFrontend *rf,
                                        const VkrUiPassPayload *payload,
  ...
  for (uint32_t i = 0; i < draw_count; ++i) {
    ...
    if (!vkr_pass_packet_resolve_draw_range(rf, submesh, false_v, &range)) {
      continue;
    }
...
vkr_internal void vkr_pass_ui_execute(VkrRgPassContext *ctx, void *user_data) {
  (void)user_data;
  ...
  const VkrUiPassPayload *payload = vkr_rg_pass_get_ui_payload(ctx);
  if (!packet || !payload) {
    return;
  }
  ...
  uint32_t width = packet->frame.window_width;
  uint32_t height = packet->frame.window_height;
  if (width == 0 || height == 0) {
    width = packet->frame.viewport_width;
    height = packet->frame.viewport_height;
  }
  if (width == 0 || height == 0) {
    return;
  }
  ...
  vkr_pass_ui_draw_list(rf, payload, payload->draws, payload->draw_count,
                        base_instance, &globals);

  if (rf->ui_system.initialized) {
    vkr_ui_system_render_text(rf, &rf->ui_system);
  }
}

bool8_t vkr_pass_ui_register(VkrRgExecutorRegistry *registry) {
  if (!registry) {
    return false_v;
  }

  VkrRgPassExecutor entry = {
      .name = string8_lit("pass.ui"),
      .execute = vkr_pass_ui_execute,
      .user_data = NULL,
  };
