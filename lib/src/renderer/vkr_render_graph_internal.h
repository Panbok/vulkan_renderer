...
/**
 * @brief Internal image resource state; one per declared/imported image in the graph.
 */
typedef struct VkrRgImage {
  ...
  uint32_t allocated_generation;       /**< Generation when textures were last allocated */
  ...
  VkrTextureOpaqueHandle imported_handle; /**< Backend handle when imported (single) */
  ...
  VkrTextureOpaqueHandle *textures;    /**< Allocated textures (one or per swapchain image) */
  uint32_t texture_count;              /**< Number of textures */
  VkrTextureHandle *texture_handles;   /**< Public handles for backend (if used) */
...
/**
 * @brief Internal buffer resource state; one per declared/imported buffer in the graph.
 */
typedef struct VkrRgBuffer {
  String8 name;                 /**< Declared name (stable) */
  VkrRgBufferDesc desc;         /**< Buffer description */
  uint32_t generation;          /**< Handle generation; bumped on recompile */
  uint32_t allocated_generation; /**< Generation when buffers were last allocated */
  ...
  VkrBufferHandle *buffers;     /**< Allocated buffers (one or per swapchain image) */
...
typedef struct VkrRgPass {
  ...
  Vector_VkrRgImageBarrier pre_image_barriers;  /**< Image barriers to record before the pass */
  Vector_VkrRgBufferBarrier pre_buffer_barriers; /**< Buffer barriers to record before the pass */

  bool8_t culled; /**< True if pass was culled (outputs unused) */

  VkrRenderPassHandle renderpass;       /**< Backend render pass (after compile) */
  VkrRenderTargetHandle *render_targets; /**< Backend render targets (color/depth) */
...
/**
 * @brief Cache entry for render pass + framebuffer; keyed by pass name and attachment hash.
 */
typedef struct VkrRgRenderTargetCacheEntry {
  ...
  VkrRenderTargetHandle *targets; /**< Cached render targets (per image index) */
 ...
 * packet is frame-local and set via vkr_rg_set_packet; must remain valid during execute.
 */
typedef struct VkrRenderGraph {
  ...
  struct s_RendererFrontend *renderer;        /**< Renderer frontend (set at execute) */
  const VkrRenderPacket *packet;             /**< Frame-local; set via vkr_rg_set_packet; valid during execute */
  ...
  Vector_VkrRgRenderTargetCacheEntry render_target_cache; /**< Cached render passes + framebuffers */
  ...
  Vector_uint32_t execution_order;            /**< Pass indices in execution order (after compile) */
  bool8_t compiled;                          /**< True after successful vkr_rg_compile */
  VkrRenderGraphResourceStats resource_stats; /**< Live/peak resource counts and bytes */
  Vector_VkrRgPassTiming pass_timings;        /**< Per-pass timing from last execute */
} VkrRenderGraph;

/**
 * @brief Adds image count and bytes to the graph's resource stats (live and peak).
...
/**
 * @brief Adds buffer count and bytes to the graph's resource stats (live and peak).
 * @param graph Render graph
...
/**
 * @brief Releases all allocated textures for an image and updates resource stats.
 * No-op for imported images (only frees the textures array). Frees texture_handles if present.
 * @param graph Render graph
...
/**
 * @brief Releases all allocated buffers for a buffer resource and updates resource stats.
 * No-op for imported buffers (only frees the buffers array).
 * @param graph Render graph
...
/**
 * @brief Returns the backend texture for an image at the given swapchain image index.
 * For single-texture or imported images, returns that texture; otherwise indexes into textures array.
...
/**
 * @brief Returns the backend buffer for a buffer resource at the given swapchain image index.
 * For single-buffer or imported buffers, returns that buffer; otherwise indexes into buffers array.
...
/**
 * @brief Clears all passes and execution order; resources and frame state are unchanged.
 * @param graph Render graph
 */
