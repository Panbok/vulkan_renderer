...
| [ADR-040](040-metalfx-temporal-dynamic-resolution.md) | MetalFX temporal reconstruction with completion-driven dynamic resolution | Accepted; Metal stages scene-linear HDR, depth, and exact previous-encode motion into a native-resolution MetalFX path, orders in-flight transforms on the GPU, and selects bounded internal tiers from completed GPU intervals. Vulkan retains the portable temporal consumer |
