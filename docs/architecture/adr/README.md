...
| [ADR-038](038-sh-l2-diffuse-irradiance.md) | Second-order spherical-harmonic normalized diffuse response replacing the baked irradiance cubemap | Partial; final packet-version-23 Metal/Vulkan implementation preserves the `E/pi` convention while cubemaps remain for source, skybox, and specular prefilter. Focused native validation and compiled-root reflection pass on both backends; matched quality, deterministic GPU-reference, lifetime-stress, and authoritative performance acceptance evidence remains open |
