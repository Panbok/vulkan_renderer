...
| [ADR-039](039-metal-internal-render-scale.md) | Metal internal scene scale with folded spatial upscale | Accepted; Metal keeps the physical present target and UI native while viewport-domain Scene work uses an explicit scale in `(0,1]`. Direct mode reconstructs to the target; paneled mode reconstructs to the dock-owned Scene output. Vulkan rejects non-unit renderer scale |
| [ADR-040](040-metalfx-temporal-dynamic-resolution.md) | MetalFX temporal reconstruction with completion-driven dynamic resolution | Accepted; direct and paneled Metal modes stage scene-linear HDR, depth, and exact previous-encode motion into a Scene-only MetalFX path, synchronize untracked textures through the public scaler fence, order in-flight transforms on the GPU, and select bounded internal tiers from completed GPU intervals. Paneled UI remains native and composites after reconstruction; Metal validation uses an explicit portable diagnostic path, and Vulkan retains the portable temporal consumer |
