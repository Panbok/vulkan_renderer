...
| [ADR-030](030-offline-mesh-optimization-and-cooking.md) | Offline mesh optimization and cooked geometry artifacts | Accepted; version-14 packed cooker/load, mandatory runtime source optimization, observability, production conversion, lifecycle stress, and glTF meshopt decode ship |
...
| [ADR-038](038-sh-l2-diffuse-irradiance.md) | Second-order spherical-harmonic normalized diffuse response replacing the baked irradiance cubemap | Partial; final packet-version-23 Metal/Vulkan implementation preserves the `E/pi` convention while cubemaps remain for source, skybox, and specular prefilter. Focused native validation, compiled-root reflection, and deterministic same-case execution pass on both backends; the numeric payload comparison, matched quality, deterministic GPU-reference, lifetime-stress, and authoritative performance acceptance evidence remain open |
