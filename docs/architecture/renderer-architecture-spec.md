...
progressive table in `shaders/shared/shadow_kernel.slangh`. At 16 taps or more a
nine-tap probe resolves fully lit and fully shadowed regions before the full
kernel runs. `vkr_shadow_light_space_origin_from_view()` converts the fitted
light-view origin into the right/up basis, including the negated X axis
