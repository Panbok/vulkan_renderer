...
as an explicit Metal-only strategy for direct and paneled Scene presentation.
In paneled mode it reconstructs only the Scene to its dock-owned output extent;
native editor UI composites afterward. ADR-027's direct retained-cache UI and
in-window docked editor architecture is implemented; its conditional persistent
UI target was declined by the prescribed early measurement.
...
└── renderer/
    ...
    ├── vkr_temporal.c/h       Backend-neutral temporal state and reset policy
...
runtime/                       Renderer-neutral sample scene and packet runtime
app/                           Renderer app entry and debug UI client
editor/                        Permanent editor entry, dock, windows, editor UI client
...
The root build has two independent application targets over a neutral sample
runtime. `vulkan_renderer` owns an F6-toggleable debug UI client. `vkr_editor`
owns the default dock composition, floating windows, and editor viewport client.
Neither executable consumes source or include paths from the other application.
There is no compile-time editor mode and F6 cannot disable the editor; its cold
`--scene-only` focus mode is selected only at startup.
...
`VkrRenderPacket` version 27 contains frame information, globals (including
...
- completion-safe `HISTORY` resource instance domains; and
...
the deferred visibility/G-buffer/HZB intermediates, the backend-neutral P19 compact-list
branch, and portable temporal transform/motion/history passes. Conditions select
resources, passes, and optional resource uses before realization. Fullscreen and
editor world output feed temporal reconstruction before tonemapping or viewport
composition; shadow cascades and HZB mip reductions expand from authored
repeats at build time.
...
- direct capture reads subresource 0 and therefore consumes that subresource's
  terminal layout; explicit whole-image exports warn when subresources disagree.
...
current version 27 retains them. A probe
...
`manual_exposure`, and `exposure_compensation_ev`, because one linear field
cannot also carry a logarithmic bias without silently changing old callers.
Packet validation now rejects a non-finite or non-positive `manual_exposure`, a
non-finite compensation bias, and an unsupported mode, so the backends carry no
substitution branch. `vkr_exposure_prepare()` derives the renderer-owned
`VkrExposureFrame` at the frontend boundary and `vkr_exposure_commit()` publishes
it only after a successful submit, mirroring the temporal contract; exposure
reuses the discontinuities `vkr_temporal_prepare()` already derived rather than
re-deriving them, and ignores projection change. Automatic mode meters the
post-temporal pre-tonemap HDR source into a 256-bin log-luminance histogram,
resolves a percentile-clipped target, and adapts through a completion-safe graph
history ring. A reset invalidates older submitted records so none can cross the
discontinuity while the reset result is in flight. Fullscreen and editor
tonemapping read the current GPU state directly before the ACES-fitted curve;
manual mode reads the validated multiplier and remains unchanged. Delayed
readback publishes the completed state, full histogram, scalar metrics, and the
multiplier used to canonicalize post-tonemap capture metadata. Scene
environments use a tagged cubemap/equirect source with all-or-fallback
activation, while local reflection probes remain cubemap-sourced. PBR materials
use constant ambient only when IBL is disabled, avoiding two contributions for
the same environment illumination.
...
version-15 packed cooked contract. Generated image and material-file publication
...
mask per cell. Finite range spheres populate it conservatively, while unbounded
...
work, then applies exact glTF attenuation. The same ordering ships in Metal and
Vulkan forward, deferred, and transmission paths. The GPU row is four `float4`
values (64 bytes). The grid grows its cell size rather than discarding
references, so broad material-merged submeshes do not impose a secondary light
cap. The 80-byte instance ABI remains stable and reuses its final three words
for temporal index, generation, and owner flags. Metrics
...
point-light instances from `bistro-lights.gltf`, and applies six exact-definition
`mesh.gltf_light_range_overrides` before activation. One override changes the
finite range of every point/spot node that references the named glTF light
definition. Malformed, duplicate, unknown, directional, non-finite,
non-positive, or uninstanced overrides fail scene preparation. This is a cold
content correction; it does not add a renderer or shader branch.

### 3.9 Portable temporal antialiasing

The selected implementations execute one same-resolution TAA contract before
tonemap and editor/UI composition. Renderer-owned state publishes an eight-step
Halton 2/3 jitter sequence, jittered and unjittered camera transforms, reset
reasons, and the newest completed history. Camera-visible opaque, cutout,
transmission, and ordinary blend use the jittered projection; culling, shadows,
HZB, and picking remain unjittered.
...
| Punctual lighting and visibility | Implemented membership and range/cone rejection; arbitrary local occlusion absent | Point and spot lights use a stable 128-light table, a conservative 384-cell fragment-local bitmask grid, and exact range/cone rejection. Metal/Vulkan forward, deferred, and transmission shaders share the four-vector light contract. Bistro calibrates the six colored string-light definitions to 5 m at its cold import boundary and retains one coherent indoor diffuse probe at intensity 1.0. The rejected hard influence-AABB experiment produced visible exterior slabs without solving diffuse leakage and is fully removed. Range and probe bounds are not geometry visibility: only the directional term samples CSM, so furniture and walls do not cast punctual shadows and diffuse wall occlusion remains absent. Evidence and the retained general-visibility limit are documented in [the indoor light-leak investigation](../rendering/bistro-indoor-light-leak-investigation.md) |
...
| glTF and scene loading | Implemented | CPU async pipeline; nested texture URIs and sidecars resolve without flattening; `EXT_meshopt_compression` buffer views decode before accessor reads; UVs lower once to VKR convention; geometric decals tagged through embedded extras or tracked per-source sidecars bake validated meter-denominated world-normal clearance before packing, while untagged geometry is unchanged; point, spot, and directional punctual lights import through the scene transform into a stable 128-light table with a fragment-local 384-cell bitmask grid. Cooked mesh entities may name `mesh.gltf_light_source` explicitly so geometry remains `.vkb` while scene-level light metadata comes from glTF. Optional `mesh.gltf_light_range_overrides` resolve exact glTF light-definition names and apply one validated finite range to every referencing point/spot node; malformed, duplicate, unknown, directional, non-finite, non-positive, and uninstanced overrides fail preparation. Required mesh dependency or GPU-publication failures fail the scene request instead of activating a meshless scene; frame-path uploads measured non-blocking |
...
| Editor viewport, docking, and picking | Implemented; native Vulkan runtime validation pending | Both packet implementations copy `editor_enabled` and the dock-derived viewport extent into graph realization. Paneled Metal separates the native drawable/UI extent, reconstructed Scene-panel extent, and internal Scene extent; MetalFX processes only Scene imagery before `Editor.Composite`, then `UI.Editor` draws at native resolution. One mapping drives camera aspect, picking, gizmos, and compositor placement. The editor's cold, opt-in scene-only mode maps that chain over the complete drawable, omits dock chrome without mutating the saved tree, and uses direct scene/UI presentation. A bounded binary dock tree supports tabs, edge/center drops, split resizing, collapse, validation, and atomic JSON persistence. Picking readback remains usually deferred, but ring wrap can block. See §8 P1 item 15 |
| UI system | Implemented direct path; cached target declined | Immediate calls build retained-ID widgets through one grid solver, previous-frame hit testing, incremental layout, input capture, Unicode text editing, and DPI-resolved styles. An identical complete visual tree reuses fixed-capacity retained CPU vertices, indices, and batches across scratch resets, skipping measure/layout, command emission, tile rebuild, and lowering; each fresh target still receives a full upload and draw. Packet v27 carries the bounded stream with ordered scissor batches; dedicated Metal/Vulkan UI pipelines render quads, bitmap/MTSDF text, and rounded rectangles. CPU tile hashes export dirty metrics, but the stream redraws directly because the prescribed local gate measured `UI.Fullscreen` at 0.017583 ms GPU p50 and retired the conditional cached target. Native Vulkan shader execution and crossed output remain open. See [ui-architecture-spec.md](../ui/ui-architecture-spec.md) and [ADR-027](adr/027-immediate-mode-grid-ui.md) |
| Text | F0–F3 core implemented; cross-platform acceptance partial | The default UI and text fixture use a reproducible one-page VKFA v1 MTSDF artifact with float em metrics, direct integer glyph lookup, glyph-ID kerning, and linear single-mip RGBA8 storage. UI/world draws carry a two-component atlas unit range; Metal and Vulkan compile the same canonical UV-derivative coverage contract for color and picking. Windows/macOS window snapshots and explicit offscreen configuration supply logical-to-device content scale before layout, while retained text transforms stay at unit scale. Native Vulkan/Windows execution, a real mixed-scale display transition, and the optional alpha-fallback A/B remain open. See §8 P1 item 16, the [text specification](../text/text-resolution-independence-and-font-cooking-spec.md), and ADR-034/035/036. |
...
| HDR/tonemap/post chain | Implemented, initial | RGBA16F fullscreen/editor scene color, packet-selected manual or automatic exposure, scene-linear bloom, full-resolution GTAO ambient visibility, ACES-fitted tonemap, and output-space FXAA share one post-temporal path before UI. Production initialization selects automatic exposure, bloom, and GTAO; explicit manual exposure and bloom/GTAO-disabled packets preserve their independent bypasses. GTAO is evaluated from current-frame depth and normals before deferred lighting and attenuates indirect diffuse only. Temporal history remains scene-linear and exposure-independent because exposure is applied only after `Temporal.Resolve`. Exposure-equivalent canonical HDR capture remains upstream. |
| Automatic exposure | Implemented through spec phase E3; matched Metal/Vulkan parity | The contract introduced in packet version 20 carries `exposure_mode`, `manual_exposure`, and `exposure_compensation_ev`; the frontend derives and submit-commits a renderer-owned frame with bounded delta and shared discontinuities. One post-temporal histogram pass meters the current fullscreen or editor HDR result into 256 log-luminance bins, then resolve computes fractional-percentile target EV and completion-proven adaptation. Valid history advances by no more than the selected EV/s rate times bounded frame delta; invalid history snaps to target. Tonemapping consumes the current GPU state without CPU synchronization. Delayed completed readback exposes the state, all histogram bins, scalar metrics, and canonical-capture multiplier. Manual output is unchanged. Static Metal and Vulkan cases are stable. Matched 1600x1200 Bistro GTAO-on and GTAO-off captures differ by `0.001169 EV` and `0.003181 EV`. |
...
| Metal internal render scale | Implemented as an explicit quality mode; authoritative performance pending | Scale in `(0,1]` keeps the physical Metal target and UI/text native while viewport-domain scene, deferred, temporal, bloom, GTAO, and transmission work use a rounded internal extent. Direct mode reconstructs to the present target; paneled mode reconstructs to the Scene panel and composites into the native drawable. Picking maps the complete target or dock panel into the internal domain. Vulkan rejects non-unit renderer scale. The harness records present extent, internal render extent, and scale; graph realization and captures expose the paneled Scene output. The M1 Pro scale-0.4 Bistro orbit clears 75 FPS at p95 in five local dirty-tree children, but authority and owner quality acceptance remain open. See ADR-039. |
| MetalFX temporal upscaling and dynamic resolution | Implemented for direct and paneled Metal Scene presentation; native validation-wrapper and quality acceptance pending | `VKR_UPSCALE_MODE_METALFX_TEMPORAL` stages internal scene-linear HDR, non-reversed depth, and normalized current-to-previous motion into private inputs sized to the Scene output, passes jitter and active content extent to one Metal 4 temporal scaler, and writes reconstructed HDR before exposure/bloom/tonemap. In paneled mode this output feeds `Editor.Composite`; `UI.Editor` follows at native drawable resolution, so panels/text never enter MetalFX. Motion targets the exact preceding scaler encode; a shared-event GPU dependency orders an in-flight transform producer, and a missing predecessor resets history. Because the inputs and output are untracked heap textures, the scaler retains the public synchronization fence required by MetalFX. An allocation-free controller consumes completed commit-feedback intervals, ignores stale-tier samples, selects 0.05 tiers plus an exact lower endpoint, and resets temporal state on every transition. The macOS sample starts at scale 0.8 with bounds `[0.334, 1.0]` and a 13.333333 ms target in either editor layout. Vulkan and zero-initialized renderer API callers retain spatial reconstruction. A fixed-scale native paneled smoke case captures both reconstructed Scene and final editor output; authoritative performance and accepted moving-image evidence remain open. Under Metal validation, the sample selects fixed-scale spatial reconstruction plus portable TAA, while explicit MetalFX requests fail cold rather than entering Apple's incompatible wrappers. See ADR-040 and §8 P1 item 18. |
...
1. Decide whether to publish new Metal and Vulkan snapshot baselines. Corrected
   ...
   generation was replaced implicitly. The former absolute exposure and GTAO
   parity issue closed on 2026-08-28 after the texture packer preserved
   tangent-space Y for BasisU BC5/EAC targets. Matched 1600x1200 GTAO-on and
   GTAO-off reports now differ by at most `0.003181 EV`; common foreground
   normals have mean dot `0.999080`. Full evidence is in the
   [Windows Vulkan post-effect parity investigation](../rendering/windows-vulkan-post-effect-parity-investigation.md).
...
3a. Complete ADR-038's retained acceptance evidence. The final SH L2 diffuse
   ...
   27 on both source backends, and the
   ...
   generation `sha256:7492b6406ad11123e0cb5f0f943f5c74bd908e3f72b13750c1a8fd1196f6e726`.
   A fresh RX 6700 XT Vulkan run compares all three captures with that payload;
   each passes with MAE `5.080678104575146e-06`, maximum absolute delta `1/255`,
   zero failed pixels, and failed-pixel ratio `0` (`sha256:549304f5a5d769943853ef46d70a281795c773ca358ff35d6bf69866790c7f32`).
   Deterministic GPU projection-fixture repetition, café-probe owner review,
   submitted-frame reload/lifetime stress, and an honest Release performance
   record remain open.
...
3c. Add geometry-aware local-light visibility where conservative authored
   range is insufficient. **Open.** The 2026-08-30 punctual influence-AABB
   experiment is rejected and reverted: a later exterior owner camera exposed
   its hard planes as colored slabs, while the indoor indirect leak remained.
   Bistro now applies six cold 5 m glTF light-definition range overrides and
   reduces its coherent indoor probe from diffuse intensity 3.5 to 1.0. Packet
   version 25 restores the four-vector, 64-byte light row and removes all AABB
   tests from CPU grid construction and Metal/Vulkan shaders. Current Release
   replays retain all 72 lights with zero drops, one probe, zero invalid
   resolves, 1,247 grid references, and at most 43 lights per cell; focused
   Metal API/GPU validation is clean. This corrects the reported Bistro views
   but does not make opaque geometry visible to point/spot lights or carry
   diffuse occlusion. Those general cases still require a budgeted,
   completion-safe punctual-shadow tier and visibility-bearing GI data. Full
   evidence is in the [Bistro indoor light-leak investigation](../rendering/bistro-indoor-light-leak-investigation.md).
...
15. ~~**The editor viewport topology is only partially connected.**~~
    **Resolved 2026-09-01; native Vulkan runtime validation remains open.** Both
    selected implementations copy the packet's editor condition and independent
    viewport extent immediately before graph realization. The authored
    viewport-domain scene images, `Editor.Composite`, and `UI.Editor` are
    reachable on both source paths. Zero viewport dimensions retain the target
    fallback.

    The dock tree owns the scene content rectangle. One pure mapping drives the
    graph viewport extent, camera aspect, window-to-target picking/gizmo
    coordinates, and one compositor viewport/scissor. The previous editor mesh
    and material representation is removed. CPU tests, Debug/Release builds,
    Release editor captures, and focused Metal API/GPU validation pass. The
    Vulkan sources and shaders compile on macOS, but the descriptor-buffer
    backend cannot execute there, so no native Vulkan runtime claim is made.

16. **The text F0–F3 core ships; native cross-platform acceptance remains
    incomplete.** A pinned offline cooker now produces one checksummed VKFA v1
    artifact from the licensed Ubuntu Mono source and explicit U+0020–U+00FF
    recipe. The cold loader validates every section and semantic reference,
    publishes the embedded linear single-mip RGBA8 page, and retains only float
    em glyph records, a sorted integer codepoint map, glyph-ID kerning, and the
    GPU atlas. The cooked face is the scalable UI default; bitmap and
    system-raster paths remain for real fixture callers. The JSON-plus-PNG
    MTSDF path remains only as a bounded Vulkan rollback.

    Metal and Vulkan compile the same canonical component-wise UV-derivative
    MTSDF reconstruction. Packet/root contracts carry the two-component atlas
    unit range, and color plus picking share coverage. Loose transition atlases
    live outside the generic packer and have no `.vkt` sidecars. Production
    uses pure RGB MSDF: the optional alpha-SDF minification blend is unaccepted
    until matched A/B captures exist on both native backends.

    Windows publishes `GetDpiForWindow() / 96`, macOS publishes backing scale,
    and offscreen cases author an explicit finite positive scale. UI applies it
    to font size, spacing, maximum dimensions, clipping, padding, and slot
    offsets before layout; no density query enters per-glyph or per-draw packet
    preparation, and the retained transform does not double-scale. Pure tests
    cover 1×, 1.25×, 1.5×, and 2×. Local Metal offscreen, resize, cold/warm
    archive, and strictly serial API/GPU-validation witnesses pass.

    Native Vulkan and Windows execution and a real mixed-scale display
    transition remain unavailable, so ADR-034/035/036 and the owning
    [text specification](../text/text-resolution-independence-and-font-cooking-spec.md)
    remain partial. The local resize captures are behavior evidence without
    accepted baselines; no baseline is accepted automatically.
...
17. **Portable TAA still lacks deformation inputs and complete cross-platform
    ...
    Manual and automatic exposure are safe across temporal discontinuities
    because both are applied by `Post.Tonemap` after scene-linear
    `Temporal.Resolve` history. Automatic adaptation resets its own
    completion-safe chain without discarding valid HDR temporal history. Any
    future pre-exposure design must add history rescaling or reset where that
...
18. **Apple's Metal 4 validation wrappers cannot currently execute MetalFX
    temporal scaling; VKR fails safe at the cold boundary.** The former
    `_outputTextureBarrierStages not set` assertion was not solely a framework
    blocker: VKR supplied untracked heap textures without the scaler's public
    synchronization fence. Every scaler now retains that fence, removing the
    assertion. The installed MetalFX framework then sends
    `globalTraceObjectID` to `MTL4DebugComputeCommandEncoder` under API
    validation and `MTL4GPUDebugComputeCommandEncoder` under GPU validation;
    neither wrapper implements the selector. The renderer rejects explicit
    MetalFX-plus-validation configurations before encoding. The sample app
    logs and uses fixed-scale spatial reconstruction with portable TAA, so
    API-only, GPU-only, and combined diagnostic launches complete without
    misreporting native MetalFX. Normal Metal 4 execution, dynamic tier changes,
    and native captures pass with validation disabled. Re-run one serialized
    focused process after an OS/Xcode update; until then, do not describe
    MetalFX itself as API- or shader-validation clean. See ADR-040.
...
- Post-processing beyond the implemented automatic exposure, scene-linear
  bloom, full-resolution GTAO, output-space FXAA, and initial ACES tonemap;
...
| Check | Result |
|---|---|
| Bistro light-leak rollback and range calibration | The punctual influence-AABB experiment is removed from scene data, CPU grid construction, both packet implementations, all Metal/Vulkan punctual loops, reflection, and tests. `./build_test.sh`, `./build_release.sh`, and `./build.sh Debug` exit 0. Two Release owner-camera profiles retain 72 lights, zero drops, one probe, zero invalid resolves, 1,247 grid references, and a maximum of 43 lights per cell (`sha256:41f1b5ad6fd218878ed05a7b64e78e4be81d0b0d2c4f5b0d77bf0b1278c85a57`, `sha256:c99421e36adbbe0bfcf76696e19e4ce7bb27436c929b0dc34b3d5cbcf325ee39`). One exclusive Debug replay passes under Metal API and GPU validation with no renderer, sanitizer, or validation error (`sha256:c4b43bbd921a4eaf419a92dd3e52ea99fd3ede5e2fd0cbd0638f6a6bc70a662e`). Exact-camera direct channels match a temporary 5 m source diagnostic byte-for-byte while the original glTF remains unchanged. These are local dirty-tree correctness observations, not an accepted baseline or performance result; native Vulkan runtime validation and owner interactive acceptance remain open |
...
| SH L2 diffuse implementation | On 2026-08-29, the full CPU suite exited 0 and both Debug and Release builds compiled the application and shader libraries. One exclusive Apple M1 Pro / Metal 4 Debug state-matrix run with Metal API and GPU/shader validation enabled passed all 11 assertions and emitted no validation diagnostic (`sha256:101be997281ecc0c81cf66984ceb2cf8c0ae0e132ae90ec0bcf7068fd7c6ded0`). A focused Metal run packed exactly one probe and emitted no validation diagnostic (`sha256:13d8ab1fe4b2a2533fc5f72130ba9115a30bffef7b0914fb424aca8f73ed3e41`). The corrected RX 6700 XT Vulkan path reflects every field of its 48-byte compiled projection root and passes two focused API plus synchronization-validation repetitions with one packed probe and no VUID/error/fatal diagnostics (`sha256:72302ea31066819c54e36e488302467a4609c1cd9b75ac0c8d327ee070d1b815`). Three visible Release `indirect_diffuse` captures are byte-identical (`sha256:a5b95003b45846451508acb29b481e6bd013945daa69b6075c8737b7aac1c64a`; report `sha256:b38e590ac8358d6ac61462109ae014bd59c6e6578374355b40950cea1f0d18c7`). The same clean Release case on Metal also packs one probe and produces three byte-identical visible captures (`sha256:183eed0d3d791e410cbfa00876aa80556aecf4135480dc538f2f705e5c0c78b1`; report `sha256:6e8431ecff0626fedb99104236941f0f4546fa0fef7a6ab1cf07c47a0eaa6451`). The full Metal payload is retained as generation `sha256:7492b6406ad11123e0cb5f0f943f5c74bd908e3f72b13750c1a8fd1196f6e726`. A fresh RX 6700 XT run compares each capture with MAE `5.080678104575146e-06`, maximum absolute delta `1/255`, zero failed pixels, and failed-pixel ratio `0`; cross-backend report `sha256:549304f5a5d769943853ef46d70a281795c773ca358ff35d6bf69866790c7f32`. Normal cold/warm cache launches pass on both backends; the Vulkan cache is 342,484 bytes. A clean authoritative Release 16-probe Metal profile measured deferred-lighting at 1.15275 ms p50 across 1,500 samples (`sha256:9d13f1159a8c813bb1c4865725d1e05fceea23156aa6538053058867ef1a9589`). It characterizes one final SH load only. Matched native quality, deterministic GPU-reference fixtures, lifetime stress, the remaining scaling cases, and valid comparative performance evidence remain open. |
| Immediate-mode grid UI and editor docking | On 2026-09-01, the full CPU suite and Debug/Release wrappers exited 0 after packet v27, the retained widget/grid/dock/input modules, and both native UI source paths were integrated. Release Metal text report `sha256:cc1417af216af5a742a8b63de1b09adb7f39d0e393bad360ba761e333ac773e4` visibly covers system, bitmap, UI MTSDF, and world MTSDF text; final color is `sha256:e4ed205d08fd028122d570198d0e8635089ca5288e4656713ab099f6d524ec88`, and picking remains UI-free at `sha256:43246e798554b457c9c9fa7298cd416bde7d73d2fae76d12e5ee3c7f09522ab9`. Editor compositor report `sha256:5aac6f2c63b44592c38e4951f1589e04f8d6d1fcc50473182241bba33b74ba80` places the scene in the dock-derived rectangle. One Debug replay enables Metal API and GPU validation and passes without a validation error (`sha256:2efdfb9418eb26d1e40ee976dcd3f3c36e2bef111740354240b32e99aa09a2bd`). The local static retained-UI profile measures `UI.Fullscreen` at 0.017583 ms GPU p50 and records report `sha256:04adf82ffb49fec44e980ca338fb413566070de860c51c3be2b96f4e32083bd5`; it is a dirty-tree scope gate, not a speed claim. Native Vulkan runtime validation and crossed output remain open. |
