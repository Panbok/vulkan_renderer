...
`VkrRenderPacket` version 20 contains frame information, globals (including
the versioned exposure mode, manual multiplier, and EV compensation), and
optional world, shadow, skybox, UI, editor, picking, text-update, and debug
payloads.
...
| HDR/tonemap/post chain | Implemented, initial | RGBA16F fullscreen/editor scene color, packet-selected manual or automatic exposure, scene-linear bloom, ACES-fitted tonemap, and output-space FXAA share one post-temporal path before UI. Production initialization selects automatic exposure and bloom; explicit manual exposure and bloom-disabled packets preserve their independent bypasses. Temporal history remains scene-linear and exposure-independent because exposure is applied only after `Temporal.Resolve`. Exposure-equivalent canonical HDR capture remains upstream. |
...
- GTAO and post-processing beyond automatic exposure, scene-linear bloom,
  output-space FXAA, and the initial ACES tonemap;
