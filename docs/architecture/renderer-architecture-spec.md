...
as an explicit Metal-only strategy. ADR-027's direct retained-cache UI and
...
| Editor viewport, docking, and picking | Implemented; native Vulkan runtime validation pending | Both packet implementations copy `editor_enabled` and the dock-derived viewport extent into graph realization. The scene renders at panel resolution, composites through one packet-owned pixel rectangle, and shares one mapping with camera aspect, picking, and gizmos. A bounded binary dock tree supports tabs, edge/center drops, split resizing, collapse, validation, and atomic JSON persistence. Picking readback remains usually deferred, but ring wrap can block. See §8 P1 item 15 |
| UI system | Implemented direct path; cached target declined | Immediate calls build retained-ID widgets through one grid solver, previous-frame hit testing, incremental layout, input capture, Unicode text editing, and DPI-resolved styles. Packet v27 carries one bounded vertex/index stream with ordered scissor batches; dedicated Metal/Vulkan UI pipelines render quads, bitmap/MTSDF text, and rounded rectangles. CPU tile hashes export dirty metrics, but the complete stream redraws directly because the prescribed local gate measured `UI.Fullscreen` at 0.017583 ms GPU p50 and retired the conditional cached target. Native Vulkan shader execution and crossed output remain open. See [ui-architecture-spec.md](../ui/ui-architecture-spec.md) and [ADR-027](adr/027-immediate-mode-grid-ui.md) |
