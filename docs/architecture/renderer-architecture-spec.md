...
provisional Metal deferred P4/P6/P8 implementation on 2026-08-13.
...
`VkrRenderPacket` version 10 contains frame information, globals (including
...
| Compute dispatch | Implemented; production Metal deferred kernels, Vulkan foundation | Typed executors carry validated direct or indirect launch descriptors without per-frame name lookup. Metal P4/P8 uses graph-declared classify, prefix, ICB encode, and G-buffer resolve kernels; Vulkan lowers the shared dispatch contract but its P5/P9 production kernels remain absent |
...
| Deferred visibility-buffer migration | Partial; Metal P4/P6/P8 provisional | P0-P3 foundations remain shared. With `VKR_DEFERRED_ENABLED=1`, Metal frustum-classifies candidates, compacts four opaque/cutout × cull-mode ranges, encodes a completion-safe ICB, runs retained forward shading from that stream, independently writes `R32G32_UINT` visibility, and resolves direct-capture diffuse, F0/roughness/occlusion, normal, emissive, and barycentric/LOD targets. Focused API and shader/GPU validation pass with 34 candidates/visible rows, bucket counts 28/6/0/0, and zero overflow or invalid resolves. The selector is disabled by default; P5/P7/P9 Vulkan parity and P10 lighting are absent, so final color remains the retained GPU-forward oracle. Captures and timings are not owner-accepted authoritative evidence. |
