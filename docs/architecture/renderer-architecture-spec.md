...
`VkrRenderPacket` version 22 contains frame information, globals (including
...
the deferred visibility/G-buffer/HZB intermediates, the Metal P19 compact-list
...
| Automatic exposure | Implemented through spec phase E3 | The contract introduced in packet version 20 carries `exposure_mode`, `manual_exposure`, and `exposure_compensation_ev`; the frontend derives and submit-commits a renderer-owned frame with bounded delta and shared discontinuities. One post-temporal histogram pass meters the current fullscreen or editor HDR result into 256 log-luminance bins, then resolve computes fractional-percentile target EV and completion-proven adaptation. Tonemapping consumes the current GPU state without CPU synchronization. Delayed completed readback exposes the state, all histogram bins, scalar metrics, and canonical-capture multiplier. Manual output is unchanged. Focused deterministic, snapshot, and Metal validation evidence passes. Vulkan's histogram input now uses the same sampled-image graph and descriptor-index contract as its shader; a two-repetition static Bistro trace holds 479,910 accepted texels and stable EV over all 120 measured frames, and focused RX 6700 XT synchronization validation passes. |
...
| GTAO | Implemented through spec phases G0-G2; authoritative performance pending | Packet version 22 adds explicit enable, radius, and power. One authored Metal/Vulkan graph slice converts current depth into the first five levels of a full-resolution R16 positive view-depth chain, evaluates a 3-slice × 3-step horizon reference into separate R8 raw visibility and edge data, and performs one edge-aware 3x3 denoise into final R8 visibility. Deferred lighting receives a branchless white fallback and multiplies only indirect diffuse after material AO; direct analytic and material-only specular-occlusion terms remain unchanged. Direct depth/normal/raw/final AO capture, odd-extent and capacity tests, two deterministic isolated-cold Release snapshots, a moving thin-occluder case, per-pass Metal timestamps, focused Metal API/GPU shader validation, and RX 6700 XT synchronization validation pass. Two isolated-warm children completed startup, but their aggregate was rejected for unrelated work-volume mismatch and is not a passed gate. |
...
0. **Close remaining absolute Metal/Vulkan exposure parity.** The structural
   Windows regressions were resolved on 2026-08-28. A shared Metal/Vulkan
   shader helper reconstructs positive
   tangent-space Z from the XY channels, so BC5 and EAC RG11 normal maps no
   longer invert surface normals. Vulkan's opaque deferred resolve now applies
   the model matrix in the same direction as Vulkan forward/transmission and
   Metal. The harness waits for the selected renderer's asset publisher before
   measurement, Vulkan teardown cancels pending IBL bake references, and all
   exposure, bloom, and GTAO smoke cases assert non-vacuous geometry with zero
   publication omissions. G-buffer normal capture version 2 emits exact
   `RG16_SNORM_LE` data. The RX 6700 XT Bistro and rotated-state-matrix reports
   pass. The Vulkan exposure histogram now receives a sampled-image descriptor
   index matching the shader array; fixed-camera traces are now stable and
   focused synchronization validation passes. Owner review then found a second,
   composition-dependent issue: at a dark Bistro camera Vulkan converges to
   `+1.579 EV` (`2.987x`), and at the supplied Metal-reference camera it
   converges to `+1.663 EV` (`3.166x`), visibly brighter than the supplied Metal
   frame. Bloom-off keeps the pre-bloom HDR digest and exposure decision
   unchanged, so bloom is not the source. Static binding and shader arithmetic
   match, but a native Metal run of the new exact-camera case is required to
   locate the divergence before any shared metering retune or backend fix.
   Full evidence and remaining baseline and residency work are in the
