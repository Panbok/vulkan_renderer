...
| Automatic exposure | Implemented through spec phase E3; absolute HDR parity open | The contract introduced in packet version 20 carries `exposure_mode`, `manual_exposure`, and `exposure_compensation_ev`; the frontend derives and submit-commits a renderer-owned frame with bounded delta and shared discontinuities. One post-temporal histogram pass meters the current fullscreen or editor HDR result into 256 log-luminance bins, then resolve computes fractional-percentile target EV and completion-proven adaptation. Valid history advances by no more than the selected EV/s rate times bounded frame delta; invalid history snaps to target. Tonemapping consumes the current GPU state without CPU synchronization. Delayed completed readback exposes the state, all histogram bins, scalar metrics, and canonical-capture multiplier. Manual output is unchanged. Static Metal and Vulkan cases are stable, but the matched Bistro camera leaves a `0.830 EV` pre-bloom HDR input difference pending the GTAO-off split. |
...
| GTAO | Implemented through spec phases G0-G2; absolute parity and authoritative performance pending | Packet version 22 adds explicit enable, radius, and power. One authored Metal/Vulkan graph slice converts current depth into the first five levels of a full-resolution R16 positive view-depth chain, evaluates a 3-slice × 3-step horizon reference into separate R8 raw visibility and edge data, and performs one edge-aware 3x3 denoise into final R8 visibility. Deferred lighting receives a branchless white fallback and multiplies only indirect diffuse after material AO; direct analytic and material-only specular-occlusion terms remain unchanged. Structural snapshots and focused validation pass. The exact Bistro camera still has a `0.830 EV` pre-bloom Metal/Vulkan gap; symmetric GTAO-off cases and GTAO-on depth/normal/visibility captures now own the remaining partition. |
...
0. **Close remaining absolute Metal/Vulkan exposure parity.** The structural
   ...
   Metal. Vulkan's former directional-shadow multiplier on diffuse IBL was also
   removed; Metal has no such term. The harness waits for the selected
   renderer's asset publisher before
   ...
   focused synchronization validation passes. The exposure adaptation contract
   now treats 3/1 EV/s as hard linear rate bounds rather than exponential
   response coefficients. A matched M1 Pro case proves both static kernels are
   stable: Metal meters `-3.309451` average log luminance and resolves
   `+0.835520 EV`, while Vulkan meters `-4.139783` and resolves `+1.665852 EV`.
   Bloom is downstream and excluded. The remaining `0.830 EV` pre-bloom HDR gap
   requires the new matched GTAO-off split before any metering retune or
   backend-only exposure compensation.
