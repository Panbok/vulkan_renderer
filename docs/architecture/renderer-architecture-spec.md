...
| Renderer automation harness | Implemented, partial | Strict cases/profiles, deterministic cameras, isolated windowed repetitions, authoritative CPU/GPU performance policies, three comparison fingerprints, metric/pass/event aggregation, atomic artifacts, and `profile`; capture, automation boot, baselines, and offscreen targets are later phases |
