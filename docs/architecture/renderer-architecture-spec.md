...
production-reference record corrected on 2026-08-01 and harness Phases 3-5
...
| Renderer automation harness | Implemented, partial | Strict cases/profiles, deterministic cameras, isolated windowed repetitions, dependency-resolved full/automation boot with actual subsystem masks, authoritative CPU/GPU/boot evidence policies, three comparison fingerprints, metric/pass/event aggregation, atomic artifacts, direct and auxiliary-debug `snapshot`, canonical color/depth/ID comparison and diffs, separated `autotest`, and profile-scoped immutable baselines with guarded promotion; true offscreen is the remaining phase |
