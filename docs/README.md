...
| Document | Status | Purpose |
|---|---|---|
| [image-quality-roadmap.md](rendering/image-quality-roadmap.md) | partial | **Ordered plan for image-quality work.** Presentation correctness, portable same-resolution TAA, stationary transparency accumulation, post-TAA FXAA, automatic exposure E0-E3, and bloom B0-B1 ship. Bloom adds packet controls, bounded odd-extent downsample/reverse-upsample chains, pre-exposure combine, and four direct debug captures on both backends. Metal runtime, timing, and validation evidence passes while native Vulkan validation remains pending. Authored deformation motion, broader animation/disocclusion fixtures, GTAO, authoritative bloom performance, and final-color acceptance remain open; MSAA is outside the implemented slice |
...
| [post-exposure-bloom-and-ambient-occlusion-spec.md](rendering/post-exposure-bloom-and-ambient-occlusion-spec.md) | partial | Roadmap items 4 and 6. Automatic exposure E0-E3 and bloom B0-B1 ship. Bloom uses explicit scene-linear controls, separate downsample/accumulation chains, reverse repeat ordering, a full-resolution pre-exposure combine, shared Metal/Vulkan arithmetic, and direct prefilter/result/pre-/post-combine capture channels. Native Vulkan validation and authoritative matched bloom performance remain open; GTAO still requires a current-frame view-space depth pyramid |
