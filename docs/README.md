...
| [bindless-renderer-audit-spec.md](architecture/bindless-renderer-audit-spec.md) | partial | Static source audit of both selected implementations and the shared GPU cores at `e028164`: how bindless each backend actually is, the Metal/Vulkan BRDF divergence, shared-core complexity and fence issues, engine-primitive alignment (`vkr_internal`, `containers/`, `vkr_atomic.h`, allocators), and a prioritized refactoring order. Recommendations 1-8 and 11 implemented 2026-08-12 with byte-identical Metal witnesses; the file split and the frame-root/draw-root ABI change remain outstanding |
