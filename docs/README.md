...
| Document | Status | Purpose |
|---|---|---|
| [image-quality-roadmap.md](rendering/image-quality-roadmap.md) | partial | **Ordered plan for image-quality work.** Presentation correctness, portable same-resolution TAA, stationary transparency accumulation, post-TAA FXAA, and automatic exposure through E3 ship. Automatic exposure has GPU histogram metering, percentile resolve, completion-safe adaptation, direct fullscreen/editor tonemap consumption, delayed diagnostics, and exposure-aware capture; Metal runtime and validation evidence passes while native Vulkan validation remains pending. Authored deformation motion, broader animation/disocclusion fixtures, bloom, and GTAO remain open; MSAA is outside the implemented slice |
...
| [post-exposure-bloom-and-ambient-occlusion-spec.md](rendering/post-exposure-bloom-and-ambient-occlusion-spec.md) | partial | Roadmap items 4 and 6. Automatic exposure E0-E3 ships: the versioned packet contract feeds GPU histogram metering, percentile resolve, completion-safe adaptation, direct fullscreen/editor tonemap consumption, delayed metrics, and completed-exposure capture metadata. Bloom downsample/upsample resources and GTAO using a current-frame view-space depth pyramid remain proposed |
