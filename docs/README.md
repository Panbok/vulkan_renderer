...
| [038](architecture/adr/038-sh-l2-diffuse-irradiance.md) | partial | Final packet-version-23 Metal/Vulkan implementation replaces the baked diffuse cubemap with GPU-resident second-order SH coefficients for `E/pi` in completion-safe slots; source, skybox, and specular-prefilter cubemaps remain. Focused native validation and compiled-root reflection pass on both backends; matched quality, deterministic GPU-reference, lifetime-stress, and authoritative performance acceptance evidence remains open |
...
| [sh-l2-diffuse-irradiance-spec.md](rendering/sh-l2-diffuse-irradiance-spec.md) | partial | Implementation and remaining evidence contract for [ADR-038](architecture/adr/038-sh-l2-diffuse-irradiance.md). SH0-SH3 and final packet version 23 are present on both source backends; focused native validation and compiled-root reflection pass on Metal and Vulkan, while matched quality, deterministic GPU-reference, lifetime-stress, and authoritative performance gates remain open |
