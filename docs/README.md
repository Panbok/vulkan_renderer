...
| Document | Status | Purpose |
|---|---|---|
| [image-quality-roadmap.md](rendering/image-quality-roadmap.md) | proposed | **Ordered plan for image-quality work.** Presentation DPI and transfer correctness now ship; the remaining plan establishes the temporal-input contract before selecting an AA implementation and uses measured temporal and MSAA prototypes to make the final quality/performance choice. D3D12, frame generation, mesh shaders, and speculative shader micro-optimization remain outside the near-term plan |
...
| [visibility-buffer-msaa-spec.md](rendering/visibility-buffer-msaa-spec.md) | proposed | Roadmap items 3 and 5. Anti-aliasing evaluation for the visibility renderer: temporal input and reset requirements, FSR 3.1 and MetalFX integration limits, and the full 4x MSAA experiment including graph, pipeline, edge-lighting, HZB, picking, blend, and transmission constraints. No AA method is accepted before the bakeoff |
| [post-exposure-bloom-and-ambient-occlusion-spec.md](rendering/post-exposure-bloom-and-ambient-occlusion-spec.md) | proposed | Roadmap items 4 and 6. Histogram auto-exposure with explicit units and history, dedicated bloom downsample/upsample resources, and GTAO using a current-frame view-space depth pyramid rather than the previous-frame occlusion HZB |
