...
| [bistro-indoor-light-leak-investigation.md](rendering/bistro-indoor-light-leak-investigation.md) | investigation | Four-camera diagnosis and Stage A correction: 54 exact-node punctual influence bounds, one coherent indoor diffuse probe after rejecting visible tiled-probe transitions, shared 96-byte GPU rows, final Metal evidence, and the remaining diffuse-visibility and Stage B punctual-shadow limits |
...
| [sh-l2-diffuse-irradiance-spec.md](rendering/sh-l2-diffuse-irradiance-spec.md) | partial | Implementation and remaining evidence contract for [ADR-038](architecture/adr/038-sh-l2-diffuse-irradiance.md). SH0-SH3 entered with packet version 23 and remain present in current version 24 on both source backends; focused native validation, compiled-root reflection, deterministic same-case execution, and the retained numeric payload comparison pass, while matched quality, deterministic GPU-reference, lifetime-stress, and authoritative performance gates remain open |
