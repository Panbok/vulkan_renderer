...
| [027](architecture/adr/027-immediate-mode-grid-ui.md) | accepted | Immediate-mode UI over a retained cache, grid-only layout, batched scissored draw list, measured CPU tile hashing, in-window docking, and a composited offscreen editor viewport; the conditional cached target was declined |
| [028](architecture/adr/028-gpu-driven-deferred-visibility-buffer.md) | implemented through P21 | GPU-driven deferred visibility buffer; both backends use one GPU-submitted topology with opaque/deferred shading, HZB, picking, four-layer transmission, and completion-gated layer coverage. The legacy world renderer is retired; both backends lower the Metal-measured eight-pass P19 compact path with a diagnostic full-screen rollback, while native Vulkan compact validation remains pending |
...
| [030](architecture/adr/030-offline-mesh-optimization-and-cooking.md) | accepted | Version-15 range-local meshoptimizer-backed `.vkb` cooking, worker validation/decode, mandatory runtime source optimization, production conversion tooling, lifecycle/byte/locality metrics, and `EXT_meshopt_compression` input decode |
...
| [039](architecture/adr/039-metal-internal-render-scale.md) | implemented | Metal internal Scene scale keeps the present target and UI native, reconstructs to either the complete target or dock-owned Scene panel, maps picking to the internal extent, and rejects non-unit renderer scale on Vulkan |
| [040](architecture/adr/040-metalfx-temporal-dynamic-resolution.md) | implemented | MetalFX temporal reconstruction writes Scene-output-resolution HDR from a bounded internal content rectangle, reprojects to the exact preceding scaler frame with GPU event ordering, synchronizes untracked inputs through the public scaler fence, and selects scale tiers from completed GPU intervals. Paneled editor UI remains native and composites afterward; Metal validation uses an explicit portable diagnostic path because Apple's wrappers remain incompatible with native MetalFX encoding |

## Rendering

| Document | Status | Purpose |
|---|---|---|
| [shader-cross-backend-contract.md](rendering/shader-cross-backend-contract.md) | partial | Complete shared, Metal, and Vulkan shader inventory with exact values, layouts, dispatches, bilateral runtime witnesses, and explicit remaining gates for draw overflow, transmission layers, direct IBL capture, SH projection, shadows, post output, and text |
...
| [presentation-dpi-and-transfer-function-spec.md](rendering/presentation-dpi-and-transfer-function-spec.md) | partial | Windows uses Per-Monitor V2 physical pixels, both backends use one linear-to-sRGB output contract, and Metal has spatial or MetalFX temporal reconstruction from an explicit internal Scene scale while output/UI remain native. Paneled Metal reconstruction is implemented; mixed-DPI fixtures, final-color and moving-quality acceptance, Vulkan scaling, and native Vulkan editor evidence remain pending |
| [visibility-buffer-msaa-spec.md](rendering/visibility-buffer-msaa-spec.md) | partial | Temporal phases T0-T2 and portable TAA ship under ADR-037; the Metal-only MetalFX T4 consumer ships under ADR-040. FSR and every visibility-buffer MSAA phase remain unimplemented |
...
| [deferred-visibility-buffer/SPEC.md](rendering/deferred-visibility-buffer/SPEC.md) | implemented through P21 | GPU-driven deferred visibility-buffer contract. Both backends use the sole GPU-submitted topology with opaque/deferred shading, HZB, picking, four-layer transmission, completion-gated coverage, and the eight-pass P19 compact path selected from Metal evidence. Native Vulkan compact validation remains pending. Rationale in [ADR-028](architecture/adr/028-gpu-driven-deferred-visibility-buffer.md) |
...
| [bistro-indoor-light-leak-investigation.md](rendering/bistro-indoor-light-leak-investigation.md) | investigation | Diagnosis and correction of the failed punctual influence-AABB experiment: exterior slabs removed, six colored glTF light-definition ranges calibrated to 5 m at cold scene import, indoor probe diffuse intensity reduced to 1.0, original mesh/glTF preserved, focused Metal validation passed, and the general punctual/diffuse visibility limits kept open |
...
| [transmission-shading-correctness-spec.md](rendering/transmission-shading-correctness-spec.md) | implemented; corrected local Metal ceiling accepted | T0-T5 plus the owner-camera correction: portable transmission lobes/textures/volume/roughness, omitted-factor glass compatibility, Metal ICB peel-root inheritance, and a capture-only fifth peel retaining ADR-028's four-layer bound. The exact Bistro panes preserve detail with decreasing layer coverage; the corrected M1 Pro ceiling is `0.400 ms` summed shade GPU mean under the five-run analytic profile. Native Vulkan validation and crossed pixels remain open |
...
| [sh-l2-diffuse-irradiance-spec.md](rendering/sh-l2-diffuse-irradiance-spec.md) | partial | Implementation and remaining evidence contract for [ADR-038](architecture/adr/038-sh-l2-diffuse-irradiance.md). SH0-SH3 entered with packet version 23 and remain present in current version 27 on both source backends; focused native validation, compiled-root reflection, deterministic same-case execution, and the retained numeric payload comparison pass, while matched quality, deterministic GPU-reference, lifetime-stress, and authoritative performance gates remain open |
...
| Document | Status | Purpose |
|---|---|---|
| [gltf-loader-design.md](assets/gltf-loader-design.md) | partial | glTF import ships with explicit geometric-decal clearance, nested texture resolution, UV orientation, and partial legacy material conversion |
...
| [meshoptimizer-geometry-pipeline-spec.md](assets/meshoptimizer-geometry-pipeline-spec.md) | implemented | Version-15 range-local cooked geometry, mandatory source optimization, packed Metal/Vulkan ABI, observability, production assets, and glTF meshopt interop |
...
| Document | Status | Purpose |
|---|---|---|
| [ui-architecture-spec.md](ui/ui-architecture-spec.md) | implemented | **Authoritative UI architecture.** Immediate-mode API over a retained cache, grid-only layout, batched scissored draw list, tile binning and hashing, Unicode input, docking, and a composited editor viewport. Rationale in [ADR-027](architecture/adr/027-immediate-mode-grid-ui.md) |
| [ui-components-library-design.md](ui/ui-components-library-design.md) | proposed | Broader component inventory under the spec; the immediate P3 widget subset ships, while retained handles, advanced components, and accessibility remain proposed |
| [ui-docking-system-design.md](ui/ui-docking-system-design.md) | proposed | Legacy broader docking design; the bounded in-window P6 split/tab tree and persistence ship, while floating windows and retained panel factories remain proposed |
