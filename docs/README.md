...
| Document | Status | Purpose |
|---|---|---|
| [shader-cross-backend-contract.md](rendering/shader-cross-backend-contract.md) | partial | Verified Metal/Vulkan shader parity ledger for reviewed values, algorithms, dispatch shapes, data layouts, native roots, and snapshot evidence. One-sided findings must be marked unaligned; unlisted shaders remain unreviewed |
