...
| [windows-vulkan-post-effect-parity-investigation.md](rendering/windows-vulkan-post-effect-parity-investigation.md) | partial | **Windows Vulkan post-effect parity investigation.** Records the BC5/EAC positive-Z normal reconstruction shared by Metal and Vulkan, corrected Vulkan exposure sampled-descriptor contract, stable fixed-camera EV evidence, corrected opaque deferred matrix direction, renderer-publication readiness gate, early IBL teardown, non-vacuous post-effect fixtures, exact `RG16_SNORM_LE` capture, and bloom isolation of the remaining dark-camera brightness. Absolute exposure parity now requires the exact-camera native Metal HDR/exposure witness before backend correction or shared metering retuning |
