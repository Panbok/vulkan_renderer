...
| L2 diffuse coefficients (ADR-038) | `shaders/shared/sh_l2_kernel.slangh` | none yet; the Metal mirror is SH2 work and the shared header is deliberately absent from the MSL concatenation list | `vulkan/slang/ibl/default.slang` `ibl_sh` entry, `vulkan/slang/common/resources.slangh` `VkrVulkanIblShRoot` | **UNALIGNED**: Vulkan-only by staging. Missing on Metal: projection kernel, evaluation call site, and pool ownership. Missing on both: the SH deferred-lighting variant that reads the slots, and every native runtime gate |
...
frame root, and every field of the transmission shade and compact roots. It
does not reflect the candidate row or every other deferred compute root.
...
| IBL bake | 32 | packet frame | 480 |
| packet draw | 48 | packet utility | 544 |
