...
// Denoise inputs share the viewport extent. A clamped texel read equals the
// former clamp-to-edge point sample at the texel center.
static float4
vkr_metal_gtao_denoise_load(texture2d<float, access::sample> texture,
                            int2 pixel, uint2 extent) {
  return texture.read(uint2(clamp(pixel, int2(0), int2(extent) - 1)));
}
...
vkr_metal_packet_gtao_denoise(constant VkrMetalPacketGtaoDenoiseRoot &root
  ...
  float4 center_edges = vkr_gtao_unpack_edges(
      vkr_metal_gtao_denoise_load(root.edges, p, root.destination_extent).x);
  float4 left_edges = vkr_gtao_unpack_edges(vkr_metal_gtao_denoise_load(
      root.edges, p + int2(-1, 0), root.destination_extent).x);
  float4 right_edges = vkr_gtao_unpack_edges(vkr_metal_gtao_denoise_load(
      root.edges, p + int2(1, 0), root.destination_extent).x);
  float4 top_edges = vkr_gtao_unpack_edges(vkr_metal_gtao_denoise_load(
      root.edges, p + int2(0, -1), root.destination_extent).x);
  float4 bottom_edges = vkr_gtao_unpack_edges(vkr_metal_gtao_denoise_load(
      root.edges, p + int2(0, 1), root.destination_extent).x);
  float4 top_left_edges = vkr_gtao_unpack_edges(vkr_metal_gtao_denoise_load(
      root.edges, p + int2(-1, -1), root.destination_extent).x);
  float4 top_right_edges = vkr_gtao_unpack_edges(vkr_metal_gtao_denoise_load(
      root.edges, p + int2(1, -1), root.destination_extent).x);
  float4 bottom_left_edges =
      vkr_gtao_unpack_edges(vkr_metal_gtao_denoise_load(
          root.edges, p + int2(-1, 1), root.destination_extent).x);
  ...
  float4 center = vkr_metal_gtao_denoise_load(root.source, p,
  ...
  for (int y = -1; y <= 1; ++y) {
    for (int x = -1; x <= 1; ++x) {
      float4 sample = vkr_metal_gtao_denoise_load(
          root.source, p + int2(x, y), root.destination_extent);
