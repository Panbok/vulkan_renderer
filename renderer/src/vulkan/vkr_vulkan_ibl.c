...
vkr_internal bool8_t vkr_vk_prepare_atmosphere_dispatch(
    ...
    const VkrVulkanPreparedIblBake *bake, uint32_t width, uint32_t height,
    uint32_t depth) {
  const VkrVulkanPublishedTexture *source = bake->source;
  const VkrVulkanPublishedTexture *transmittance = bake->transmittance;
  const VkrVulkanPublishedTexture *multiple_scattering =
      bake->multiple_scattering;
  if (!source || !source->storage_slot_count || !transmittance ||
      !transmittance->storage_slot_count || !multiple_scattering ||
      !multiple_scattering->storage_slot_count ||
  ...
  *root = (VkrVulkanAtmosphereRoot){
      ...
      .transmittance_sample = transmittance->sampled_slot.index,
      .transmittance_storage = transmittance->storage_slots[0].index,
      .multiple_scattering_sample = multiple_scattering->sampled_slot.index,
      .multiple_scattering_storage =
          multiple_scattering->storage_slots[0].index,
...
/* Makes one generation's lookup writes visible to the later dispatches of
   this bake and, in submission order, to every later frame that samples it. */
vkr_internal void
vkr_vk_record_atmosphere_lookup_barrier(VkCommandBuffer command,
                                        const VkrVulkanImage *image) {
  vkr_vk_cmd_ibl_image_barrier(
      command, image->handle, 0u, 1u, VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT,
...
/* The renderer-owned cloud noise (ADR-074) is generated by the first
   submitted IBL bake; a cancelled frame leaves it pending. */
bool8_t vkr_vk_prepare_cloud_noise(VkrVulkanRenderer *renderer,
                                   VkrVulkanPreparedIbl *prepared) {
  prepared->generate_cloud_noise = false_v;
  if (!renderer->cloud_noise_pending)
    return true_v;
  VkrVulkanFrameSlot *slot =
      &renderer->frame_slots[renderer->active_frame_slot];
  uint64_t root_address = 0u;
  VkrVulkanCloudNoiseRoot *root = vkr_vk_frame_upload_allocate(
      slot, sizeof(*root), _Alignof(VkrVulkanCloudNoiseRoot), &root_address,
      NULL);
  if (!root)
    return false_v;
  *root = (VkrVulkanCloudNoiseRoot){
      .base_texture = renderer->cloud_noise_storage_slots[0].index,
      .detail_texture = renderer->cloud_noise_storage_slots[1].index,
      .weather_texture = renderer->cloud_noise_storage_slots[2].index,
  };
  const VkrVulkanDeferredPipeline pipelines[VKR_VULKAN_CLOUD_NOISE_COUNT] = {
      VKR_VULKAN_DEFERRED_PIPELINE_CLOUD_BASE_NOISE,
      VKR_VULKAN_DEFERRED_PIPELINE_CLOUD_DETAIL_NOISE,
  ...
  for (uint32_t i = 0u; i < VKR_VULKAN_CLOUD_NOISE_COUNT; ++i) {
    /* The volumes run 4x4x4 groups and the weather map 8x8. */
    const bool8_t volume = i < 2u;
    const uint32_t group = volume ? 4u : 8u;
    VkrVulkanPreparedCompute *dispatch = &prepared->cloud_noise[i];
    *dispatch = (VkrVulkanPreparedCompute){
        .root_address = root_address,
        .pipelines = {renderer->deferred_pipelines[pipelines[i]]},
        .groups = {{(sizes[i] + group - 1u) / group,
                    (sizes[i] + group - 1u) / group,
                    volume ? (sizes[i] + group - 1u) / group : 1u}},
        .dispatch_count = 1u,
    };
  }
  prepared->generate_cloud_noise = true_v;
  return true_v;
}

void vkr_vk_record_cloud_noise(VkrVulkanRenderer *renderer,
                               VkCommandBuffer command,
                               const VkrVulkanPreparedIbl *prepared) {
  if (!prepared->generate_cloud_noise)
    return;
  for (uint32_t i = 0u; i < VKR_VULKAN_CLOUD_NOISE_COUNT; ++i) {
    const VkImage image = renderer->cloud_noise_images[i].handle;
    vkr_vk_cmd_ibl_image_barrier(
        command, image, 0u, 1u, VK_PIPELINE_STAGE_2_NONE, VK_ACCESS_2_NONE,
        VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT,
        VK_ACCESS_2_SHADER_STORAGE_WRITE_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
        VK_IMAGE_LAYOUT_GENERAL);
    vkr_vk_record_prepared_compute(renderer, command,
                                   &prepared->cloud_noise[i]);
    /* Visible to the cloud passes of this and every later submission. */
    vkr_vk_cmd_ibl_image_barrier(
        command, image, 0u, 1u, VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT,
        VK_ACCESS_2_SHADER_STORAGE_WRITE_BIT,
        VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT |
            VK_PIPELINE_STAGE_2_FRAGMENT_SHADER_BIT,
        VK_ACCESS_2_SHADER_SAMPLED_READ_BIT, VK_IMAGE_LAYOUT_GENERAL,
  ...
  renderer->frame_slots[renderer->active_frame_slot].cloud_noise_recorded =
      true_v;
}

void vkr_vk_commit_cloud_noise(VkrVulkanRenderer *renderer,
                               VkrVulkanFrameSlot *slot) {
  if (!slot->cloud_noise_recorded)
    return;
  renderer->cloud_noise_pending = false_v;
  slot->cloud_noise_recorded = false_v;
}

bool8_t vkr_vk_prepare_ibl_bakes(VkrVulkanRenderer *renderer,
                                 VkrVulkanPreparedIbl *prepared) {
  prepared->clear_coefficients = !renderer->sh_coefficients_cleared;
  if (!vkr_vk_prepare_cloud_noise(renderer, prepared))
    return false_v;
  ...
  for (uint32_t i = 0u; i < renderer->pending_ibl_bake_count; ++i) {
    ...
    VkrVulkanPublishedTexture *transmittance =
        job->is_atmosphere
            ? vkr_vk_texture_publication(renderer, job->transmittance)
            : NULL;
    VkrVulkanPublishedTexture *multiple_scattering =
        job->is_atmosphere
            ? vkr_vk_texture_publication(renderer, job->multiple_scattering)
            : NULL;
    if (!source || !prefilter ||
        (job->is_atmosphere && (!transmittance || !multiple_scattering)))
      return false_v;
    if (source->initialization_pending ||
        (job->is_atmosphere && (transmittance->initialization_pending ||
                                multiple_scattering->initialization_pending))) {
      continue;
    }
    ...
    bake->multiple_scattering = multiple_scattering;
    ...
    if (job->is_atmosphere) {
      ...
      if (!vkr_vk_prepare_atmosphere_dispatch(
              ...
              VKR_VULKAN_ATMOSPHERE_PIPELINE_TRANSMITTANCE, job, bake,
...
void vkr_vk_record_ibl_bakes(VkrVulkanRenderer *renderer,
                             VkCommandBuffer command,
                             const VkrVulkanPreparedIbl *prepared) {
  vkr_vk_record_cloud_noise(renderer, command, prepared);
  ...
  for (uint32_t i = 0u; i < prepared->count; ++i) {
    ...
    if (bake->is_atmosphere) {
      /* Writable initialization already moved both fresh lookup images to
         GENERAL for compute storage writes. */
      vkr_vk_record_prepared_compute(renderer, command, &bake->atmosphere[0]);
      vkr_vk_record_atmosphere_lookup_barrier(command,
                                              &bake->transmittance->image);
      vkr_vk_record_prepared_compute(renderer, command, &bake->atmosphere[1]);
      vkr_vk_record_atmosphere_lookup_barrier(
          command, &bake->multiple_scattering->image);
