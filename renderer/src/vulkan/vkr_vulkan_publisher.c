...
vkr_internal bool8_t vkr_vk_upload_prepared_texture(
  ...
  *out_error = VKR_RENDERER_ERROR_RESOURCE_CREATION_FAILED;
  ...
  if (!valid_layers ||
    ...
    log_error("Vulkan rejected texture type %u with %u layers",
              prepared->description.type, prepared->upload_array_layers);
    return false_v;
  }
  const VkImageViewType view_type = cube         ? VK_IMAGE_VIEW_TYPE_CUBE
                                    : cube_array ? VK_IMAGE_VIEW_TYPE_CUBE_ARRAY
                                    : array      ? VK_IMAGE_VIEW_TYPE_2D_ARRAY
...
vkr_internal bool8_t vkr_vk_asset_publish_geometry_internal(
    ...
    const VkrGeometryUploadRange *submeshes, uint32_t submesh_count) {
  VkrVulkanRenderer *renderer = state;
  if (!renderer || !geometry || handle.id == 0u ||
      ...
      renderer->staging_buffer_count >
          renderer->retired_staging_buffer_capacity - 3u ||
      ...
      (uint64_t)geometry->decode_count >
  ...
  if (record->live) {
    if (record->handle.generation != handle.generation ||
        ...
        record->decode_count != geometry->decode_count ||
    ...
    for (uint32_t i = 0u; i < submesh_count; ++i) {
      record->submeshes[i] = (VkrVulkanSubmeshRange){
          ...
          .decode_index = submeshes[i].decode_index,
      };
    }
  ...
  const uint64_t vertex_size =
      (uint64_t)sizeof(VkrPackedStaticVertex) * geometry->vertex_count;
  const uint64_t decode_size =
      (uint64_t)geometry->decode_count * sizeof(VkrGpuGeometryDecodeRecord);
  ...
  VkrVulkanPendingBufferInitialization initializations[3] = {0};
  const bool8_t created =
      vkr_vk_prepare_published_upload(
          renderer, geometry->vertices, vertex_size, &mega->vertices,
          vertex_offset,
          VK_PIPELINE_STAGE_2_VERTEX_SHADER_BIT |
              VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT,
          ...
          &initializations[0]) &&
      vkr_vk_prepare_published_upload(
          renderer, geometry->decodes, decode_size, &mega->vertices,
          decode_offset,
          VK_PIPELINE_STAGE_2_VERTEX_SHADER_BIT |
              VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT,
          VK_ACCESS_2_SHADER_STORAGE_READ_BIT, handle.id - 1u,
          &initializations[1]) &&
      vkr_vk_prepare_published_upload(
          renderer, geometry->indices, index_size, &mega->indices, index_offset,
          VK_PIPELINE_STAGE_2_INDEX_INPUT_BIT |
              VK_PIPELINE_STAGE_2_VERTEX_SHADER_BIT |
              VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT,
          VK_ACCESS_2_INDEX_READ_BIT | VK_ACCESS_2_SHADER_STORAGE_READ_BIT,
          handle.id - 1u, &initializations[2]);
  if (!created) {
    vkr_vk_release_buffer_initialization(renderer, &initializations[2]);
  ...
  pending.gpu_row = (VkrGpuGeometryRow){
      ...
      .first_vertex = (uint32_t)(vertex_offset / sizeof(VkrPackedStaticVertex)),
      ...
      .flags = 1u,
      .decode_address = pending.vertices.address + decode_offset,
  };
  pending.live = true_v;
  *record = pending;
  ...
  renderer->pending_buffer_initializations
      [renderer->pending_buffer_initialization_count++] = initializations[2];
  vkr_vk_advance_candidate_publication_generation(renderer);
  return true_v;
}
...
vkr_internal bool8_t vkr_vk_asset_publish_loaded_mesh(
  ...
  if (!mesh || !mesh->submeshes || !mesh->submesh_count ||
      mesh->submesh_count >
          (UINT32_MAX >> VKR_INSTANCE_TEMPORAL_SURFACE_SHIFT) ||
      mesh->buffer.vertex_size != sizeof(VkrPackedStaticVertex) ||
      mesh->buffer.vertex_layout != VKR_GPU_VERTEX_LAYOUT_STATIC_PACKED_V1 ||
      !mesh->buffer.decodes || mesh->buffer.decode_count == 0 ||
      mesh->buffer.index_size != sizeof(uint32_t))
    return false_v;
  for (uint64_t i = 0; i < mesh->submesh_count; ++i) {
    const VkrGeometryUploadRange *range = &mesh->submeshes[i];
    if (!range->index_count || range->first_index > mesh->buffer.index_count ||
        range->index_count > mesh->buffer.index_count - range->first_index)
      return false_v;
  }
  const VkrGeometryConfig geometry = {
      .vertex_size = mesh->buffer.vertex_size,
      .vertex_count = mesh->buffer.vertex_count,
      .vertices = mesh->buffer.vertices,
      .vertex_layout = mesh->buffer.vertex_layout,
      .decodes = mesh->buffer.decodes,
      .decode_count = mesh->buffer.decode_count,
      .index_size = mesh->buffer.index_size,
      .index_count = mesh->buffer.index_count,
      .indices = mesh->buffer.indices,
  };
  return vkr_vk_asset_publish_geometry_internal(
      state, handle, &geometry, mesh->submeshes, (uint32_t)mesh->submesh_count);
}
