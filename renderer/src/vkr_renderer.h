 ...
 */
typedef enum VkrCaptureFeature {
  VKR_CAPTURE_FEATURE_NONE = 0,
  VKR_CAPTURE_FEATURE_SHADOWS,
  VKR_CAPTURE_FEATURE_PICKING,
} VkrCaptureFeature;
...
typedef struct VkrCaptureChannelDescription {
  ...
  VkrCaptureFeature required_feature;
...
/** Allocates the opaque renderer once. The allocator object/context and native
 * surface handles remain valid through release; assets must be unpublished
 * first. A failed creation frees instance storage and preserves the
...
/** Releases an instance returned by vkr_renderer_create(). Waits for GPU
 * completion, destroys native state, then frees instance storage. */
void vkr_renderer_release(VkrRenderer *renderer);
struct VkrAssetPublisher;
/** Borrowed publisher; its renderer must outlive every asset using it. */
const struct VkrAssetPublisher *
