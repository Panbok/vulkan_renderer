...
bool8_t vkr_renderer_metrics_register(VkrRendererMetrics *renderer_metrics,
  ...
  const char *hzb_rejection_names[VKR_HZB_HISTORY_REJECTION_COUNT] = {
      ...
      "visibility.hzb.history_rejection.incomplete",
      "visibility.hzb.history_rejection.world",
      "visibility.hzb.history_rejection.extent",
      "visibility.hzb.history_rejection.camera",
      "visibility.hzb.history_rejection.raster",
  };
...
#define VKR_REGISTER_PACKET_BUILD_NS(FIELD, NAME)                              \
  ...
  VKR_REGISTER_PACKET_BUILD_NS(graph_build, "cpu.graph_build");
  VKR_REGISTER_PACKET_BUILD_NS(graph_compile, "cpu.graph_compile");
...
#define VKR_SET_F64(FIELD, VALUE)                                              \
  ...
  if (packet_build->valid && packet_build->graph_timing_valid) {
    vkr_metrics_duration_add_ns(metrics, ids->graph_build,
                                packet_build->graph_build_ns);
    vkr_metrics_duration_add_ns(metrics, ids->graph_compile,
                                packet_build->graph_compile_ns);
  } else {
    vkr_metrics_mark(metrics, ids->graph_build,
                     VKR_METRIC_AVAILABILITY_UNAVAILABLE,
                     VKR_METRIC_REASON_NOT_SAMPLED);
    vkr_metrics_mark(metrics, ids->graph_compile,
                     VKR_METRIC_AVAILABILITY_UNAVAILABLE,
                     VKR_METRIC_REASON_NOT_SAMPLED);
  }
...
#define VKR_SET_DELTA(FIELD, BASELINE, VALUE)                                  \
  ...
  if (world->gpu_diagnostics_valid) {
    ...
    VKR_SET_U64(visibility_gpu_bucket_opaque_single,
                world->gpu_bucket_counts[0] + world->gpu_bucket_counts[4]);
    VKR_SET_U64(visibility_gpu_bucket_opaque_double,
                world->gpu_bucket_counts[1] + world->gpu_bucket_counts[5]);
    VKR_SET_U64(visibility_gpu_bucket_cutout_single,
                world->gpu_bucket_counts[2] + world->gpu_bucket_counts[6]);
    VKR_SET_U64(visibility_gpu_bucket_cutout_double,
                world->gpu_bucket_counts[3] + world->gpu_bucket_counts[7]);
  ...
  VKR_SET_U64(material_texture_stream_demanded_missing,
              context->application.material_texture_stream_demanded_missing);
  VKR_SET_U64(material_texture_stream_demanded_evicted,
              context->application.material_texture_stream_demanded_evicted);
  VKR_SET_U64(material_texture_stream_in_flight,
              context->application.material_texture_stream_in_flight);
  VKR_SET_U64(material_texture_stream_resident,
              context->application.material_texture_stream_resident);
  VKR_SET_U64(material_texture_stream_evicted,
              context->application.material_texture_stream_evicted);
  VKR_SET_U64(material_texture_stream_resident_bytes,
              context->application.material_texture_stream_resident_bytes);
  VKR_SET_U64(material_texture_stream_budget_bytes,
              context->application.material_texture_stream_budget_bytes);
  VKR_SET_U64(material_texture_stream_applied,
              context->application.material_texture_stream_applied);
  VKR_SET_U64(material_texture_stream_failed,
              context->application.material_texture_stream_failed);
  VKR_SET_U64(material_texture_stream_evicted_total,
              context->application.material_texture_stream_evicted_total);
  VKR_SET_U64(material_texture_stream_pressure_stalls,
              context->application.material_texture_stream_pressure_stalls);
  VKR_SET_U64(
