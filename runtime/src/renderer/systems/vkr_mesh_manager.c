...
vkr_internal bool8_t vkr_mesh_manager_publish_loaded_mesh(
  ...
  if (!mesh_result->has_mesh_buffer || !mesh_result->submeshes.data ||
      mesh_result->submeshes.length == 0u ||
      mesh_result->submeshes.length >
  ...
  const uint32_t range_count = (uint32_t)mesh_result->submeshes.length;
  const VkrMeshLoaderBuffer *buffer = &mesh_result->mesh_buffer;
  const VkrGeometryUpload upload = {
      .buffer =
          {
              ...
              .vertex_layout = buffer->vertex_layout,
              .decodes = buffer->decodes,
              .decode_count = buffer->decode_count,
          },
      .submeshes = mesh_result->submeshes.data,
      .submesh_count = range_count,
  };
