...
static bool8_t vkr_font_cooked_parse_layout(const uint8_t *data, uint64_t size,
  ...
  if (!vkr_f32_is_binary32() || !data || !out ||
      size < VKR_FONT_COOKED_DATA_OFFSET ||
      size > VKR_FONT_COOKED_MAX_FILE_SIZE) {
    return false_v;
  }
  if (vkr_load_le_u32(data + VKR_FONT_COOKED_H_MAGIC) !=
          VKR_FONT_COOKED_MAGIC ||
      vkr_load_le_u32(data + VKR_FONT_COOKED_H_VERSION) !=
          VKR_FONT_COOKED_VERSION ||
      vkr_load_le_u32(data + VKR_FONT_COOKED_H_ENDIAN) !=
          VKR_FONT_COOKED_ENDIAN_TAG ||
      vkr_load_le_u32(data + VKR_FONT_COOKED_H_SIZE) !=
  ...
  const uint32_t stored_header_crc =
      vkr_load_le_u32(header + VKR_FONT_COOKED_HEADER_CRC_OFFSET);
  MemZero(header + VKR_FONT_COOKED_HEADER_CRC_OFFSET, sizeof(uint32_t));
  if (stored_header_crc != vkr_crc32(header, sizeof(header))) {
    return false_v;
  }
  const uint64_t file_size =
      vkr_load_le_u64(data + VKR_FONT_COOKED_H_FILE_SIZE);
  if (file_size != size || file_size < VKR_FONT_COOKED_DATA_OFFSET) {
    return false_v;
  }
  if (vkr_load_le_u32(data + 60u) != 0u || vkr_load_le_u32(data + 128u) != 0u) {
    return false_v;
  }
  ...
  out->flags = vkr_load_le_u32(data + VKR_FONT_COOKED_H_FLAGS);
  out->field_kind = vkr_load_le_u32(data + VKR_FONT_COOKED_H_FIELD);
  out->fallback_glyph_id = vkr_load_le_u32(data + VKR_FONT_COOKED_H_FALLBACK);
  out->cooker_version = vkr_load_le_u32(data + VKR_FONT_COOKED_H_COOKER);
  out->glyph_count = vkr_load_le_u32(data + VKR_FONT_COOKED_H_GLYPHS);
  out->codepoint_count = vkr_load_le_u32(data + VKR_FONT_COOKED_H_CODEPOINTS);
  out->kerning_count = vkr_load_le_u32(data + VKR_FONT_COOKED_H_KERNINGS);
  out->page_count = vkr_load_le_u32(data + VKR_FONT_COOKED_H_PAGES);
  out->file_size = file_size;
  out->face_size = vkr_load_le_u32(data + VKR_FONT_COOKED_H_FACE_SIZE);
  MemCopy(out->identity, data + VKR_FONT_COOKED_H_IDENTITY, 32u);
  uint8_t *metrics = (uint8_t *)data + VKR_FONT_COOKED_H_METRICS;
  out->metrics.line_height = vkr_load_le_f32(metrics + 0u);
  out->metrics.ascender = vkr_load_le_f32(metrics + 4u);
  out->metrics.descender = vkr_load_le_f32(metrics + 8u);
  out->metrics.underline_y = vkr_load_le_f32(metrics + 12u);
  out->metrics.underline_thickness = vkr_load_le_f32(metrics + 16u);
  out->metrics.distance_range = vkr_load_le_f32(metrics + 20u);
  out->metrics.atlas_px_per_em = vkr_load_le_f32(metrics + 24u);
  out->metrics.units_per_em = vkr_load_le_u32(metrics + 28u);
  const uint32_t stored_directory_crc =
      vkr_load_le_u32(data + VKR_FONT_COOKED_DIRECTORY_CRC_OFFSET);
  if (stored_directory_crc != vkr_crc32(data + VKR_FONT_COOKED_DIRECTORY_OFFSET,
                                        VKR_FONT_COOKED_DIRECTORY_SIZE)) {
    return false_v;
  }
  const uint32_t stored_payload_crc =
      vkr_load_le_u32(data + VKR_FONT_COOKED_PAYLOAD_CRC_OFFSET);
  if (stored_payload_crc != vkr_crc32(data + VKR_FONT_COOKED_DATA_OFFSET,
                                      size - VKR_FONT_COOKED_DATA_OFFSET)) {
    return false_v;
  }
  for (uint32_t i = 0; i < VKR_FONT_COOKED_SECTION_COUNT; ++i) {
    ...
    section->kind = vkr_load_le_u32(entry);
    section->offset = vkr_load_le_u64(entry + 8u);
    section->size = vkr_load_le_u64(entry + 16u);
    section->crc = vkr_load_le_u32(entry + 24u);
    if (section->kind != i + 1u || vkr_load_le_u32(entry + 4u) != 0u ||
        vkr_load_le_u32(entry + 28u) != 0u ||
        ...
        section->crc != vkr_crc32(data + section->offset, section->size)) {
      return false_v;
    }
  }
  ...
  for (uint32_t i = 0; i < VKR_FONT_COOKED_SECTION_COUNT; ++i) {
    const uint64_t expected_offset =
        vkr_align_up_u64(payload_end, VKR_FONT_COOKED_ALIGNMENT);
    ...
    if (!vkr_checked_add_u64(out->sections[i].offset, out->sections[i].size,
                             &section_end)) {
      return false_v;
    }
    payload_end = section_end;
  }
  const uint64_t aligned_payload_end =
      vkr_align_up_u64(payload_end, VKR_FONT_COOKED_ALIGNMENT);
  ...
  uint64_t expected = 0;
  if (!vkr_checked_mul_u64(out->glyph_count, VKR_FONT_COOKED_GLYPH_SIZE,
                           &expected) ||
      out->sections[1].size != expected ||
      !vkr_checked_mul_u64(out->codepoint_count, VKR_FONT_COOKED_CODEPOINT_SIZE,
                           &expected) ||
      out->sections[2].size != expected ||
      !vkr_checked_mul_u64(out->kerning_count, VKR_FONT_COOKED_KERNING_SIZE,
                           &expected) ||
      out->sections[3].size != expected ||
      !vkr_checked_mul_u64(out->page_count, VKR_FONT_COOKED_PAGE_SIZE,
                           &expected) ||
...
static bool8_t vkr_font_cooked_find_glyph(const uint8_t *data,
  ...
  while (count != 0u) {
    ...
    const uint32_t candidate =
        vkr_load_le_u32(records + (uint64_t)index * VKR_FONT_COOKED_GLYPH_SIZE);
...
vkr_font_cooked_validate_semantics(const uint8_t *data,
  ...
  for (uint32_t i = 0; i < layout->glyph_count; ++i) {
    ...
    const uint32_t id = vkr_load_le_u32(g);
    const uint32_t page = vkr_load_le_u32(g + 4u);
    const uint32_t flags = vkr_load_le_u32(g + 8u);
    float32_t values[9];
    for (uint32_t j = 0; j < 9u; ++j) {
      values[j] = vkr_load_le_f32(g + 12u + j * 4u);
  ...
  for (uint32_t i = 0; i < layout->codepoint_count; ++i) {
    ...
    const uint32_t codepoint = vkr_load_le_u32(cp);
    if ((i != 0u && codepoint <= previous) || codepoint > 0x10ffffu ||
        (codepoint >= 0xd800u && codepoint <= 0xdfffu) ||
        !vkr_font_cooked_find_glyph(data, layout, vkr_load_le_u32(cp + 4u))) {
      return false_v;
    }
    previous = codepoint;
  }
  ...
  for (uint32_t i = 0; i < layout->kerning_count; ++i) {
    ...
    const uint32_t left = vkr_load_le_u32(k);
    const uint32_t right = vkr_load_le_u32(k + 4u);
    if ((i != 0u &&
        ...
        !vkr_font_cooked_finite(vkr_load_le_f32(k + 8u)) ||
        vkr_load_le_u32(k + 12u) != 0u) {
      return false_v;
    }
    prior_left = left;
    prior_right = right;
  ...
  for (uint32_t i = 0; i < layout->page_count; ++i) {
    ...
    const uint32_t width = vkr_load_le_u32(p);
    const uint32_t height = vkr_load_le_u32(p + 4u);
    const uint32_t stride = vkr_load_le_u32(p + 8u);
    const uint32_t format = vkr_load_le_u32(p + 12u);
    const uint64_t offset = vkr_load_le_u64(p + 16u);
    const uint64_t pixel_size = vkr_load_le_u64(p + 24u);
    if (width == 0u || height == 0u ||
        ...
        vkr_load_le_u32(p + 36u) != 0u || vkr_load_le_u32(p + 40u) != 0u ||
        vkr_load_le_u32(p + 44u) != 0u ||
        vkr_load_le_u32(p + 32u) != vkr_crc32(data + offset, pixel_size)) {
      return false_v;
    }
...
bool8_t vkr_font_cooked_decode(VkrAllocator *result_allocator,
  ...
  for (uint32_t i = 0; i < decoded.glyph_count; ++i) {
    ...
    g->glyph_id = vkr_load_le_u32(src);
    g->page_index = vkr_load_le_u32(src + 4u);
    g->flags = vkr_load_le_u32(src + 8u);
    g->advance = vkr_load_le_f32(src + 12u);
    g->plane_left = vkr_load_le_f32(src + 16u);
    g->plane_bottom = vkr_load_le_f32(src + 20u);
    g->plane_right = vkr_load_le_f32(src + 24u);
    g->plane_top = vkr_load_le_f32(src + 28u);
    g->uv_left = vkr_load_le_f32(src + 32u);
    g->uv_bottom = vkr_load_le_f32(src + 36u);
    g->uv_right = vkr_load_le_f32(src + 40u);
    g->uv_top = vkr_load_le_f32(src + 44u);
  }
  records = data + layout.sections[2].offset;
  for (uint32_t i = 0; i < decoded.codepoint_count; ++i) {
    decoded.codepoints[i].codepoint = vkr_load_le_u32(records + i * 8u);
    decoded.codepoints[i].glyph_id = vkr_load_le_u32(records + i * 8u + 4u);
  }
  records = data + layout.sections[3].offset;
  for (uint32_t i = 0; i < decoded.kerning_count; ++i) {
    decoded.kernings[i].left_glyph_id = vkr_load_le_u32(records + i * 16u);
    decoded.kernings[i].right_glyph_id =
        vkr_load_le_u32(records + i * 16u + 4u);
    decoded.kernings[i].amount = vkr_load_le_f32(records + i * 16u + 8u);
  }
  records = data + layout.sections[4].offset;
  for (uint32_t i = 0; i < decoded.page_count; ++i) {
    VkrFontCookedPage *p = &decoded.pages[i];
    p->width = vkr_load_le_u32(records);
    p->height = vkr_load_le_u32(records + 4u);
    p->row_stride = vkr_load_le_u32(records + 8u);
    p->pixel_format = (VkrFontCookedPixelFormat)vkr_load_le_u32(records + 12u);
    const uint64_t pixel_offset = vkr_load_le_u64(records + 16u);
    p->pixel_size = vkr_load_le_u64(records + 24u);
