...
set "BUILD_DIR=build_bindless_vulkan_v3_%BUILD_TYPE%"
...
cmake --build "%BUILD_DIR%" --target vkr_bindless_vulkan_v3 --config %BUILD_TYPE%
if errorlevel 1 exit /b 1

set "V3_EXE=%CD%\%BUILD_DIR%\tools\vkr_bindless_vulkan_v3.exe"
if not exist "!V3_EXE!" set "V3_EXE=%CD%\%BUILD_DIR%\tools\%BUILD_TYPE%\vkr_bindless_vulkan_v3.exe"
echo Built !V3_EXE!
endlocal
