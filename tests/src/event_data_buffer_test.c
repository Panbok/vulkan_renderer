...
void test_event_data_buffer_create_destroy(void) {
  ...
  EventDataBuffer edb;
  bool8_t created =
      event_data_buffer_create(arena, DEFAULT_TEST_BUFFER_CAPACITY, &edb);
...
void test_event_data_buffer_alloc_simple(void) {
  ...
  bool8_t allocated = event_data_buffer_alloc(&edb, payload_size, &payload_ptr);
...
void test_event_data_buffer_alloc_zero_size(void) {
  ...
  event_data_buffer_create(arena, DEFAULT_TEST_BUFFER_CAPACITY, &edb);
  void *payload_ptr = NULL;
  bool8_t allocated = event_data_buffer_alloc(&edb, 0, &payload_ptr);
...
void test_event_data_buffer_alloc_full(void) {
  ...
  event_data_buffer_create(arena, small_cap, &edb);
  void *ptr1 = NULL;
  uint64_t size1 = 10; // needs sizeof(header) + 10
  assert(event_data_buffer_alloc(&edb, size1, &ptr1) && "First alloc failed");
  ...
  assert(!event_data_buffer_alloc(&edb, size2, &ptr2) &&
...
void test_event_data_buffer_alloc_wrap_around(void) {
  ...
  event_data_buffer_create(arena, cap, &edb);
  void *ptr1, *ptr2, *ptr3;
  ...
  assert(event_data_buffer_alloc(&edb, size1, &ptr1));
  assert(event_data_buffer_free(
  ...
  event_data_buffer_destroy(&edb); // Reset EDB for clearer test
  event_data_buffer_create(arena, cap, &edb);
  ...
  assert(event_data_buffer_free(&edb, size1));
  ...
  assert(event_data_buffer_alloc(&edb, size_wrap, &ptr3));
...
void test_event_data_buffer_alloc_fragmented(void) {
  ...
  assert(event_data_buffer_alloc(&edb, 10, &p1));
  // Alloc 2 (15 -> block 23) -> tail 41, fill 18+23=41
  assert(event_data_buffer_alloc(&edb, 15, &p2));
  // Free 1 (10 -> block 18) -> head 18, fill 41-18=23. Tail still 41.
  assert(event_data_buffer_free(&edb, 10));
  ...
  assert(!event_data_buffer_alloc(&edb, 20, &p3) &&
...
void test_event_data_buffer_free_simple(void) {
  ...
  void *ptr = NULL;
  uint64_t size = 20;
  assert(event_data_buffer_alloc(&edb, size, &ptr));
  uint64_t initial_fill = edb.fill;
  assert(event_data_buffer_free(&edb, size));
...
void test_event_data_buffer_free_empty_buffer(void) {
  ...
  assert(event_data_buffer_free(&edb, 0) &&
  ...
  uint64_t size = 10;
  assert(event_data_buffer_alloc(&edb, size, &ptr));
  assert(event_data_buffer_free(&edb, size)); // Buffer becomes empty
  assert(edb.fill == 0);
  assert(event_data_buffer_free(&edb, size) &&
...
void test_event_data_buffer_free_consistency_checks(void) {
  ...
  assert(event_data_buffer_alloc(&edb, actual_size, &ptr));
...
void test_event_data_buffer_rollback_simple(void) {
  ...
  event_data_buffer_rollback_last_alloc(&edb);
...
void test_event_data_buffer_rollback_to_empty(void) {
  ...
  assert(event_data_buffer_alloc(&edb, s1, &p1)); // fill=bs1, tail=bs1
  assert(event_data_buffer_alloc(&edb, s2, &p2)); // fill=bs1+bs2, tail=bs1+bs2
  assert(edb.last_alloc_block_size == bs2);

  event_data_buffer_rollback_last_alloc(&edb); // Rollback p2
  ...
  event_data_buffer_destroy(&edb);
  event_data_buffer_create(arena, DEFAULT_TEST_BUFFER_CAPACITY, &edb);
  assert(event_data_buffer_alloc(&edb, s1,
                                 &p1)); // Sets last_alloc_block_size to bs1
  event_data_buffer_rollback_last_alloc(&edb); // Rollback p1
...
void test_event_data_buffer_complex_interleave(void) {
  ...
  event_data_buffer_create(arena, 100, &edb);
  void *p1, *p2, *p3, *p4, *p5;
  uint64_t s1 = 10, s2 = 20, s3 = 5, s4 = 15, s5 = 25;
  uint64_t bs1 = 18, bs2 = 28, bs3 = 13, bs4 = 23, bs5 = 33;

  // 1. Alloc s1, s2
  assert(event_data_buffer_alloc(&edb, s1, &p1)); // tail=18, fill=18, last=18
  assert(event_data_buffer_alloc(&edb, s2,
                                 &p2)); // tail=46, fill=18+28=46, last=28
  assert(edb.tail == bs1 + bs2 && edb.fill == bs1 + bs2);

  // 2. Free s1
  assert(event_data_buffer_free(&edb, s1)); // head=18, fill=28 (p2 remains)
  ...
  // 9. Free s5
  assert(event_data_buffer_free(&edb, s5)); // head=0, tail=0, fill=0
  assert(edb.fill == 0 && edb.head == 0 && edb.tail == 0);
