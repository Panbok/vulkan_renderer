...
static void test_capture_pending_ready_release(void) {
  ...
  VkrCaptureRing ring = {0};
  assert(vkr_capture_ring_init(&ring, 2, 512, storage, sizeof(storage)));
  ...
  assert(vkr_capture_ring_poll(&ring, request.request_id, 5, &poll) ==
  ...
  assert(vkr_capture_ring_release(&ring, request.request_id));
  assert(vkr_capture_ring_poll(&ring, request.request_id, 5, &poll) ==
...
static void test_capture_capacity_and_abandoned_retirement(void) {
  ...
  VkrCaptureRing ring = {0};
  assert(vkr_capture_ring_init(&ring, 2, 256, storage, sizeof(storage)));
  ...
  third.request_id = 12;
  assert(vkr_capture_ring_reserve(&ring, &first, &plan, 1) ==
         VKR_RENDERER_ERROR_NONE);
  assert(vkr_capture_ring_submit(&ring, first.request_id, 7, staging));
  assert(vkr_capture_ring_release(&ring, first.request_id));
  assert(vkr_capture_ring_reserve(&ring, &second, &plan, 2) ==
         VKR_RENDERER_ERROR_NONE);
  assert(vkr_capture_ring_reserve(&ring, &third, &plan, 3) ==
         VKR_RENDERER_ERROR_CAPTURE_BUSY);
  vkr_capture_ring_collect(&ring, 7);
  assert(vkr_capture_ring_reserve(&ring, &third, &plan, 3) ==
         VKR_RENDERER_ERROR_NONE);
  assert(vkr_capture_ring_release(&ring, second.request_id));
  assert(vkr_capture_ring_release(&ring, third.request_id));
...
static void test_capture_failure_and_validation(void) {
  ...
  VkrCaptureRing ring = {0};
  assert(vkr_capture_ring_init(&ring, 1, 64, storage, sizeof(storage)));
  VkrCaptureItemRequest item = {.channel = 0};
  VkrCaptureBatchRequest request = {
      .request_id = 4, .items = &item, .item_count = 1};
  VkrCaptureBackendItemPlan oversized = capture_plan(0, 0, 65);
  assert(vkr_capture_ring_reserve(&ring, &request, &oversized, 0) ==
         VKR_RENDERER_ERROR_OUT_OF_MEMORY);
  VkrCaptureBackendItemPlan plan = capture_plan(0, 0, 16);
  assert(vkr_capture_ring_reserve(&ring, &request, &plan, 9) ==
         VKR_RENDERER_ERROR_NONE);
  assert(vkr_capture_ring_fail(&ring, request.request_id,
                               VKR_RENDERER_ERROR_COMMAND_RECORDING_FAILED));
  VkrCapturePollResult poll = {0};
  assert(vkr_capture_ring_poll(&ring, request.request_id, 0, &poll) ==
