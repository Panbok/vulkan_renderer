...
static bool32_t simd_vector_equals(VKR_SIMD_F32X4 a, VKR_SIMD_F32X4 b,
                                   float32_t epsilon) {
  return (vkr_abs_f32(a.x - b.x) < epsilon) &&
         (vkr_abs_f32(a.y - b.y) < epsilon) &&
         (vkr_abs_f32(a.z - b.z) < epsilon) &&
         (vkr_abs_f32(a.w - b.w) < epsilon);
}
...
static void test_simd_load_store(void) {
  ...
  assert(float_equals(v.x, 1.0f, VKR_FLOAT_EPSILON) &&
         "Load failed for x component");
  assert(float_equals(v.y, 2.0f, VKR_FLOAT_EPSILON) &&
         "Load failed for y component");
  assert(float_equals(v.z, 3.0f, VKR_FLOAT_EPSILON) &&
         "Load failed for z component");
  assert(float_equals(v.w, 4.0f, VKR_FLOAT_EPSILON) &&
         "Load failed for w component");

  vkr_simd_store_f32x4(output, v);

  assert(float_equals(output[0], 1.0f, VKR_FLOAT_EPSILON) &&
         "Store failed for element 0");
  assert(float_equals(output[1], 2.0f, VKR_FLOAT_EPSILON) &&
         "Store failed for element 1");
  assert(float_equals(output[2], 3.0f, VKR_FLOAT_EPSILON) &&
         "Store failed for element 2");
  assert(float_equals(output[3], 4.0f, VKR_FLOAT_EPSILON) &&
...
static void test_simd_set(void) {
  ...
  assert(float_equals(v1.x, 1.5f, VKR_FLOAT_EPSILON) &&
         "vkr_simd_set_f32x4 failed for x");
  assert(float_equals(v1.y, 2.5f, VKR_FLOAT_EPSILON) &&
         "vkr_simd_set_f32x4 failed for y");
  assert(float_equals(v1.z, 3.5f, VKR_FLOAT_EPSILON) &&
         "vkr_simd_set_f32x4 failed for z");
  assert(float_equals(v1.w, 4.5f, VKR_FLOAT_EPSILON) &&
         "vkr_simd_set_f32x4 failed for w");

  // Test vkr_simd_set1_f32x4 (broadcast)
  VKR_SIMD_F32X4 v2 = vkr_simd_set1_f32x4(7.0f);
  assert(float_equals(v2.x, 7.0f, VKR_FLOAT_EPSILON) &&
         "vkr_simd_set1_f32x4 failed for x");
  assert(float_equals(v2.y, 7.0f, VKR_FLOAT_EPSILON) &&
         "vkr_simd_set1_f32x4 failed for y");
  assert(float_equals(v2.z, 7.0f, VKR_FLOAT_EPSILON) &&
         "vkr_simd_set1_f32x4 failed for z");
  assert(float_equals(v2.w, 7.0f, VKR_FLOAT_EPSILON) &&
         "vkr_simd_set1_f32x4 failed for w");

  // Test element access patterns
  assert(float_equals(v1.r, 1.5f, VKR_FLOAT_EPSILON) &&
         "Color access (r) failed");
  assert(float_equals(v1.g, 2.5f, VKR_FLOAT_EPSILON) &&
         "Color access (g) failed");
  assert(float_equals(v1.b, 3.5f, VKR_FLOAT_EPSILON) &&
         "Color access (b) failed");
  assert(float_equals(v1.a, 4.5f, VKR_FLOAT_EPSILON) &&
         "Color access (a) failed");

  assert(float_equals(v1.s, 1.5f, VKR_FLOAT_EPSILON) &&
         "Texture access (s) failed");
  assert(float_equals(v1.t, 2.5f, VKR_FLOAT_EPSILON) &&
         "Texture access (t) failed");
  assert(float_equals(v1.p, 3.5f, VKR_FLOAT_EPSILON) &&
         "Texture access (p) failed");
  assert(float_equals(v1.q, 4.5f, VKR_FLOAT_EPSILON) &&
         "Texture access (q) failed");

  assert(float_equals(v1.elements[0], 1.5f, VKR_FLOAT_EPSILON) &&
         "Array access [0] failed");
  assert(float_equals(v1.elements[1], 2.5f, VKR_FLOAT_EPSILON) &&
         "Array access [1] failed");
  assert(float_equals(v1.elements[2], 3.5f, VKR_FLOAT_EPSILON) &&
         "Array access [2] failed");
  assert(float_equals(v1.elements[3], 4.5f, VKR_FLOAT_EPSILON) &&
...
static void test_simd_arithmetic(void) {
  ...
  assert(simd_vector_equals(add_result, expected_add, VKR_FLOAT_EPSILON) &&
  ...
  assert(simd_vector_equals(sub_result, expected_sub, VKR_FLOAT_EPSILON) &&
  ...
  assert(simd_vector_equals(mul_result, expected_mul, VKR_FLOAT_EPSILON) &&
  ...
  assert(simd_vector_equals(div_result, expected_div, VKR_FLOAT_EPSILON) &&
...
static void test_simd_sqrt(void) {
  ...
  assert(simd_vector_equals(sqrt_result, expected_sqrt, VKR_FLOAT_EPSILON) &&
  ...
  assert(simd_vector_equals(sqrt_ones, ones, VKR_FLOAT_EPSILON) &&
         "sqrt(1) failed");
...
static void test_simd_min_max(void) {
  ...
  assert(simd_vector_equals(min_result, expected_min, VKR_FLOAT_EPSILON) &&
  ...
  assert(simd_vector_equals(max_result, expected_max, VKR_FLOAT_EPSILON) &&
  ...
  assert(simd_vector_equals(neg_min, expected_neg_min, VKR_FLOAT_EPSILON) &&
  ...
  assert(simd_vector_equals(neg_max, expected_neg_max, VKR_FLOAT_EPSILON) &&
...
static void test_simd_fma(void) {
  ...
  assert(simd_vector_equals(fma_result, expected_fma, VKR_FLOAT_EPSILON) &&
  ...
  assert(simd_vector_equals(fms_result, expected_fms, VKR_FLOAT_EPSILON) &&
  ...
  assert(simd_vector_equals(fnma_result, expected_fnma, VKR_FLOAT_EPSILON) &&
  ...
  assert(simd_vector_equals(fnms_result, expected_fnms, VKR_FLOAT_EPSILON) &&
...
static void test_simd_dot_products(void) {
  ...
  assert(float_equals(hadd_result, expected_hadd, VKR_FLOAT_EPSILON) &&
  ...
  assert(float_equals(dot4_result, expected_dot4, VKR_FLOAT_EPSILON) &&
  ...
  assert(float_equals(orthogonal_dot, 0.0f, VKR_FLOAT_EPSILON) &&
         "Orthogonal vectors dot product failed");

  float32_t unit_dot = vkr_simd_dot_f32x4(unit_x, unit_x);
  assert(float_equals(unit_dot, 1.0f, VKR_FLOAT_EPSILON) &&
...
static void test_simd_compare(void) {
  ...
  VKR_SIMD_F32X4 near_lanes =
      vkr_simd_set_f32x4(1.0005f, 1.9994f, 3.0f, 4.0003f);
  ...
  assert(!vkr_simd_compare_f32x4(base, far_lanes, VKR_SIMD_COMPARE_MODE_EQUAL,
                                 0.0f) &&
...
static void test_simd_shuffle(void) {
  ...
  assert(simd_vector_equals(identity, v, VKR_FLOAT_EPSILON) &&
  ...
  assert(simd_vector_equals(reverse, expected_reverse, VKR_FLOAT_EPSILON) &&
  ...
  assert(simd_vector_equals(broadcast, expected_broadcast, VKR_FLOAT_EPSILON) &&
...
static void test_simd_edge_cases(void) {
  ...
  VKR_SIMD_F32X4 tiny = vkr_simd_set1_f32x4(VKR_FLOAT_EPSILON);
  VKR_SIMD_F32X4 tiny_add = vkr_simd_add_f32x4(tiny, tiny);
  VKR_SIMD_F32X4 expected_tiny_add =
      vkr_simd_set1_f32x4(2.0f * VKR_FLOAT_EPSILON);
  assert(simd_vector_equals(tiny_add, expected_tiny_add,
                            VKR_FLOAT_EPSILON / 10.0f) &&
  ...
  assert(simd_vector_equals(abs_like, expected_abs, VKR_FLOAT_EPSILON) &&
  ...
  assert(simd_vector_equals(zero_result, zero, VKR_FLOAT_EPSILON) &&
  ...
  assert(simd_vector_equals(one_result, any_value, VKR_FLOAT_EPSILON) &&
...
static void test_simd_scatter_gather(void) {
  ...
  assert(simd_vector_equals(gather_identity, source, VKR_FLOAT_EPSILON) &&
  ...
  VKR_SIMD_F32X4 expected_reverse =
      vkr_simd_set_f32x4(40.0f, 30.0f, 20.0f, 10.0f);
  assert(
      simd_vector_equals(gather_reverse, expected_reverse, VKR_FLOAT_EPSILON) &&
  ...
  assert(simd_vector_equals(gather_dup, expected_dup, VKR_FLOAT_EPSILON) &&
  ...
  VKR_SIMD_F32X4 expected_mixed =
      vkr_simd_set_f32x4(20.0f, 40.0f, 10.0f, 30.0f);
  assert(simd_vector_equals(gather_mixed, expected_mixed, VKR_FLOAT_EPSILON) &&
  ...
  assert(simd_vector_equals(gather_oob, expected_oob, VKR_FLOAT_EPSILON) &&
  ...
  assert(simd_vector_equals(scatter_identity, source, VKR_FLOAT_EPSILON) &&
  ...
  assert(simd_vector_equals(scatter_dup, expected_scatter_dup,
                            VKR_FLOAT_EPSILON) &&
  ...
  assert(simd_vector_equals(scatter_oob, expected_scatter_oob,
                            VKR_FLOAT_EPSILON) &&
...
static void test_simd_scatter_gather_edge_cases(void) {
  ...
  assert(simd_vector_equals(gather_zero, zero_values, VKR_FLOAT_EPSILON) &&
         "Gather with zero values failed");

  VKR_SIMD_F32X4 scatter_zero =
      vkr_simd_scatter_f32x4(zero_values, valid_indices);
  assert(simd_vector_equals(scatter_zero, zero_values, VKR_FLOAT_EPSILON) &&
  ...
  assert(
      simd_vector_equals(gather_negative, negative_values, VKR_FLOAT_EPSILON) &&
      "Gather with negative values failed");

  VKR_SIMD_F32X4 scatter_negative =
      vkr_simd_scatter_f32x4(negative_values, valid_indices);
  assert(simd_vector_equals(scatter_negative, negative_values,
                            VKR_FLOAT_EPSILON) &&
  ...
  assert(simd_vector_equals(gathered, original, VKR_FLOAT_EPSILON) &&
  ...
  assert(simd_vector_equals(scattered_perm, original, VKR_FLOAT_EPSILON) &&
